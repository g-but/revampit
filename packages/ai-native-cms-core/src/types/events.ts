...
export interface AINativeCMSEvents {
  ...
  'suggestion:updated': {
    suggestion: import('./index').Suggestion;
    changes: Partial<import('./index').Suggestion>;
  };
  'suggestion:status_changed': {
    suggestion: import('./index').Suggestion;
    oldStatus: import('./index').SuggestionStatus;
  };
  ...
  'notification:sent': {
    provider: string;
    payload: import('./index').NotificationPayload;
    success: boolean;
  };
  'notification:failed': {
    provider: string;
