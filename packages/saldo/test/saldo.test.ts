...
test('worked scheduled days balance to zero; extra day goes positive', () => {
  ...
  const extra = [
    ...entries,
    { work_date: '2026-07-02', duration_minutes: 480, category: 'repair' },
  ];
...
test('paid absence fulfils Soll, unpaid does not', () => {
  ...
  assert.equal(
    computeTimeSaldo({
      ...base,
      entries: [{ work_date: '2026-07-01', duration_minutes: 480, category: 'krank' }],
    })!.saldoMinutes,
    0,
  );
  assert.equal(
    computeTimeSaldo({
      ...base,
      entries: [{ work_date: '2026-07-01', duration_minutes: 480, category: 'unbezahlt' }],
    })!.saldoMinutes,
    -480,
  );
});
...
test('a public holiday on a scheduled day reduces Soll and ignores a manual Feiertag entry', () => {
  ...
  const base = {
    openingMinutes: 0,
    openingDate: null,
    periods: [{ validFrom: '2026-01-01', weeklyMinutes: 2400 }],
    schedule: parseWeeklySchedule(null),
    holidays,
    today: '2026-01-02',
  };
  assert.equal(computeTimeSaldo({ ...base, entries: [] })!.saldoMinutes, 0);
  assert.equal(
    computeTimeSaldo({
      ...base,
      entries: [{ work_date: '2026-01-01', duration_minutes: 480, category: 'feiertag' }],
    })!.saldoMinutes,
    0,
  );
});
...
test('applies a Pensum change from its valid_from date', () => {
  const r = computeTimeSaldo({
    ...
    periods: [
      ...
      { validFrom: '2026-07-13', weeklyMinutes: 48 * 60 },
    ],
    schedule: moDiMiSchedule(),
    entries: [],
    holidays: new Set<string>(),
...
test('a report month BEFORE the opening date still shows real month Soll/Ist (no fake 0)', () => {
  ...
  const entries = ['2026-06-01', '2026-06-02', '2026-06-03'].map((d) => ({
    work_date: d,
...
test('custom category config: a bespoke unpaid category is excluded', () => {
  ...
  const opts = {
    categories: {
      ...
      classifyAbsence: (c: string) => (c === 'sabbatical' ? { paid: false } : undefined),
    },
  };
  assert.equal(
    computeTimeSaldo(
      {
        ...base,
        entries: [{ work_date: '2026-07-01', duration_minutes: 480, category: 'sabbatical' }],
      },
      opts,
    )!.saldoMinutes,
    -480,
...
test('vacation: pro-rates the default entitlement by Pensum (60% → 12 days)', () => {
  const r = computeVacationBalance({
