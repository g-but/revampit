...
const handleOld = JSON.parse(
  fs.readFileSync(path.join(ROOT, 'packages/saldo/package.json'), 'utf8'),
).name;
if (!displayOld || !handleOld) {
  console.error('Could not read the current name from brand.ts / package.json.');
  process.exit(1);
}
const slug = displayNew
  .toLowerCase()
  .replace(/[^a-z0-9]+/g, '-')
  .replace(/^-|-$/g, '');
...
edit('packages/saldo/src/brand.ts', (s) =>
  repl(s, `name: '${displayOld}'`, `name: '${displayNew}'`),
);
edit('src/app/[locale]/projects/data.ts', (s) =>
  repl(s, `brandName: '${displayOld}'`, `brandName: '${displayNew}'`),
);

// 2) npm handle everywhere it's imported (unambiguous string — safe to replace)
for (const f of [
  'packages/saldo/package.json',
  'tsconfig.json',
  'jest.config.js',
  'src/lib/team/saldo.ts',
]) {
...
for (const loc of ['de', 'en', 'es', 'fr', 'it', 'ja', 'ko', 'ru']) {
  ...
  let touched = false;
  for (const it of j?.projects?.items ?? [])
    if (it.title === displayOld) {
      it.title = displayNew;
      touched = true;
    }
  if (touched) {
