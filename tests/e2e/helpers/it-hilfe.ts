...
export function buildTestRequestPayload(
  overrides: Partial<ItHilfeRequestPayload> = {},
): ItHilfeRequestPayload {
  return {
    categoryId: 'laptop',
...
export async function createItHilfeRequest(
  ...
  const body = preferredTechnicianId ? { ...payload, preferredTechnicianId } : payload;
...
export async function withdrawItHilfeOffer(
  ...
  const response = await csrfDelete(
    request,
    `/api/it-hilfe/requests/${requestId}/offers/${offerId}`,
  );
