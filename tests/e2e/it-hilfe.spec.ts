...
test.describe('IT-Hilfe hub (/it-hilfe)', () => {
  ...
  test('shows hub with three journey paths', async ({ page }) => {
    await expect(page.getByRole('heading', { level: 1 })).toContainText(
      /Computer reparieren|IT-Probleme/,
    );
    await expect(page.getByRole('link', { name: /Anfrage starten/i })).toHaveAttribute(
      'href',
      /\/it-hilfe\/create/,
    );
    await expect(page.getByRole('link', { name: /Techniker finden/i })).toHaveAttribute(
      'href',
      /\/it-hilfe\/techniker/,
    );
    await expect(page.getByRole('link', { name: /Anfragen ansehen/i })).toHaveAttribute(
      'href',
      /\/it-hilfe\/anfragen/,
    );
  });
...
test.describe('IT-Hilfe full journey (API + UI)', () => {
  ...
  test('create → offer → accept → complete → review', async ({ page }) => {
    ...
    await expect(
      page
        .getByRole('button', { name: 'Angebot abgeben' })
        .or(page.getByText(/Angebot abgegeben|bereits ein Angebot/i)),
    ...
    await page.reload();
    await expect(page.getByText(/bewertet|reviewed|Danke/i).first()).toBeVisible({
      timeout: 15000,
    });
  });
});
