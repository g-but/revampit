...
test.describe('Dashboard timecards — submit flow', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    test.skip(
      !hasSession,
      'Set AUTH_TEST_EMAIL + AUTH_TEST_PASSWORD, or run: npm run test:e2e:auth',
    );
  });
  ...
  test('fills the month and submits for review', async ({ page }) => {
    ...
    const submitButton = page.getByRole('button', {
      name: /Zur Prüfung einreichen|Erneut einreichen/,
    });
    ...
    await expect(
      page.getByText(/Freigabe-Team wird benachrichtigt|Zur Prüfung gesendet/i),
    ).toBeVisible({
      timeout: 20000,
    });
