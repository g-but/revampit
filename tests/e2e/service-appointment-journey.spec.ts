...
test.describe('Service appointment dual-persona journey', () => {
  ...
  test('admin customer books → assigns user techniker → both see appointment → cancel', async ({
    ...
    try {
      ...
      expect((await fetchServiceAppointment(page.request, appointmentId)).status).toMatch(
        /requested|pending/,
      );
      ...
      expect((await fetchServiceAppointment(page.request, appointmentId)).status).toMatch(
        /accepted|assigned|requested|pending/,
      );
      ...
      expect((await fetchServiceAppointment(page.request, appointmentId)).repairer_id).toBe(
        technikerUserId,
      );
      ...
      if (appointmentId) {
        try {
          ...
          const current = await fetchServiceAppointment(page.request, appointmentId).catch(
            () => null,
          );
