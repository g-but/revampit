...
import {
  reviewTimecard,
  getTimecardByIdForReview,
  saveTimecardEntriesForReview,
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'timecard_payroll_locked') {
        return apiBadRequest(
          'Diese Zeitkarte ist in einem Lohnlauf gesperrt und kann nicht geändert werden.',
        );
      }
...
export const PATCH = withAdmin(
  ...
  async (request: NextRequest, session: ValidSession, context) => {
    try {
      ...
      if (error instanceof Error && error.message === 'timecard_payroll_locked') {
        return apiBadRequest(
          'Diese Zeitkarte ist in einem Lohnlauf gesperrt und kann nicht bearbeitet werden.',
        );
      }
      if (error instanceof Error && error.message === 'timecard_not_submitted') {
        return apiBadRequest(
          'Diese Zeitkarte ist nicht mehr zur Freigabe eingereicht. Bitte neu laden.',
        );
      }
