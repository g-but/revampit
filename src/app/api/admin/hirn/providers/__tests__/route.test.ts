...
vi.mock('@/lib/hirn/providers', () => ({
  ...
  updateProviderSettings: (...args: unknown[]) => mockUpdateProviderSettings(...args),
...
const MOCK_PROVIDER_SETTINGS = [
  {
    ...
    is_enabled: true,
    is_default: true,
    settings: { model: 'llama-3.3-70b', description: 'Groq fast' },
  },
  {
    provider: 'ollama',
    is_enabled: false,
    is_default: false,
    settings: { model: 'llama3.2', base_url: 'http://localhost:11434', description: 'Local' },
  },
];
...
describe('PATCH /api/admin/hirn/providers — validation', () => {
  ...
  it('returns 400 when setting disabled provider as default', async () => {
    ...
    const response = await PATCH(
      makePatchRequest({ provider: 'ollama', isDefault: true, isEnabled: false }),
    );
    expect(response.status).toBe(400);
  });
