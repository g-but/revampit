  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        suspended_at: sql<
          string | null
        >`CASE WHEN ${repairerProfiles.status} = 'suspended' THEN ${repairerProfiles.updatedAt} ELSE NULL END`,
        ...
        skills: sql<
          string[] | null
        >`(SELECT array_agg(us.skill_id) FROM ${sql.raw(TABLE_NAMES.USER_SKILLS)} us WHERE us.user_id = ${repairerProfiles.userId})`,
      })
