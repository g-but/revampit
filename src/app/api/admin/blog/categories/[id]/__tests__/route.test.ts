...
vi.mock('@/db/schema', () => ({
  blogCategories: {
    ...
    description: 'bc_desc',
    color: 'bc_color',
    sortOrder: 'bc_sortOrder',
    updatedAt: 'bc_updatedAt',
  },
  blogPosts: {
    id: 'bp_id',
...
function makeRequest(method = 'GET', body?: Record<string, unknown>) {
  return new NextRequest(
    'http://localhost/api/admin/blog/categories/cat-1',
    body
      ? {
          method: method,
...
describe('PATCH /api/admin/blog/categories/[id] — validation', () => {
  ...
  it('returns 400 when slug conflicts with another category', async () => {
    ...
    const response = await PATCH(
      makeRequest('PATCH', { name: 'Other', slug: 'software' }),
      makeContext(),
    );
    expect(response.status).toBe(400);
  });
...
describe('PATCH /api/admin/blog/categories/[id] — success', () => {
  it('returns 200 with updated category', async () => {
    ...
    const response = await PATCH(
      makeRequest('PATCH', { name: 'Repair Updated', slug: 'repair' }),
      makeContext(),
    );
    expect(response.status).toBe(200);
  });
  ...
  it('returns the updated data', async () => {
    ...
    mockSelectWhere.mockResolvedValueOnce([MOCK_CATEGORY]).mockResolvedValueOnce([]);
...
describe('DELETE /api/admin/blog/categories/[id] — success', () => {
  ...
  it('returns 500 when DB throws', async () => {
    mockSelectWhere.mockResolvedValueOnce([MOCK_CATEGORY]).mockResolvedValueOnce([{ count: 0 }]);
