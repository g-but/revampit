...
vi.mock('@/db/schema', () => ({
  blogSubmissions: {
    ...
    editHistory: 'bs_editHistory',
    lastEditedBy: 'bs_lastEditedBy',
    lastEditedAt: 'bs_lastEditedAt',
    ...
    reviewNotes: 'bs_reviewNotes',
    rejectionReason: 'bs_rejectionReason',
    publishedPostId: 'bs_publishedPostId',
...
vi.mock('@/lib/services/blog-submission', () => ({
  approveSubmission: (...args: unknown[]) => mockApproveSubmission(...args),
  rejectSubmission: (...args: unknown[]) => mockRejectSubmission(...args),
  publishSubmission: (...args: unknown[]) => mockPublishSubmission(...args),
  requestChanges: (...args: unknown[]) => mockRequestChanges(...args),
  editSubmission: (...args: unknown[]) => mockEditSubmission(...args),
...
describe('PATCH /api/admin/blog/submissions/[id] — success', () => {
  it('returns 200 on approve action', async () => {
    ...
    const response = await PATCH(
      makeRequest('PATCH', { action: 'approve', review_notes: 'LGTM' }),
      makeContext(),
    );
    expect(response.status).toBe(200);
