...
const mockValidateCurrentFocus = vi.fn();
...
vi.mock('@/db/schema', () => ({
  teamProfiles: {
    id: 'tp_id',
    userId: 'tp_userId',
    currentFocus: 'tp_currentFocus',
    currentFocusUpdatedAt: 'tp_currentFocusUpdatedAt',
...
vi.mock('@/lib/schemas/activity', () => ({
  validateCurrentFocus: (...args: unknown[]) => mockValidateCurrentFocus(...args),
}));
...
beforeEach(() => {
  ...
  mockValidateCurrentFocus.mockReturnValue({
    success: true,
    data: { current_focus: 'Working on auth refactor' },
  });
...
describe('PUT /api/admin/team/profiles/[id]/focus — success', () => {
  it('returns 200 on success', async () => {
    const response = await PUT(
      makeRequest({ current_focus: 'Working on auth refactor' }),
      makeContext(),
    );
