...
vi.mock('@/db/schema', () => ({
  refunds: {
    ...
    refundTransactionId: 'r_refundTransactionId',
  ...
  paymentTransactions: {
    ...
    providerId: 'pt_providerId',
    providerTransactionId: 'pt_providerTransactionId',
  },
...
vi.mock('@/lib/payments/payrexx-client', () => ({
  refundTransaction: (...args: unknown[]) => mockRefundTransaction(...args),
}));
...
const MOCK_REFUND = {
  ...
  provider_transaction_id: 'payrexx-tx-123',
  currency: 'CHF',
  provider_id: 'prov-1',
  provider_slug: 'payrexx',
};
...
beforeEach(() => {
  ...
  mockValidateBody.mockReturnValue({
    success: true,
    data: { action: 'approve', notes: undefined },
  });
});
...
describe('PUT /api/admin/refunds/[id] — success', () => {
  it('returns 200 on approve action', async () => {
    ...
    mockWhere.mockResolvedValueOnce([MOCK_REFUND]).mockResolvedValueOnce([MOCK_UPDATED]);
    const response = await PUT(makeRequest('PUT', { action: 'approve' }), makeContext());
    expect(response.status).toBe(200);
  });
