...
export const POST = withAdmin('intake', async (request, session) => {
  try {
    ...
    const intakeTier = destination ? getTierForDestination(destination) : data.intake_tier;
    const publishUntested = destination ? isUntestedShopDestination(destination) : false;
    ...
    const result = await db.transaction(async (tx) => {
      const created = await createErfassungProduct(
        ...
        tx,
        {
          ...
          donation:
            !data.existing_donation_id && data.is_donation
              ? {
...
export const GET = withAdmin('intake', async (request) => {
  try {
    ...
    const baseWhere =
      baseConditions.length > 0 ? sql`WHERE ${sql.join(baseConditions, sql` AND `)}` : sql``;
    ...
    const items = (itemsResult.rows as IntakeRow[]).map((row) => {
      if (!row.intake_tier) {
        return {
          ...row,
          checklist_progress: NO_CHECKLIST_PROGRESS,
