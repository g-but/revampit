  ...
  try {
    ...
    if (row.ai_product_id) {
      const [img] = await db
        ...
        .from(productImages)
        .where(
          and(eq(productImages.productId, row.ai_product_id), eq(productImages.isPrimary, true)),
        )
        .limit(1);
    ...
    return apiSuccess({
      ...
      checklist_grouped: Object.entries(grouped).map(([category, items]) => ({
        category,
        label:
          CHECKLIST_CATEGORY_LABELS[category as keyof typeof CHECKLIST_CATEGORY_LABELS] || category,
        items,
      })),
