...
export const GET = withAdmin('finanzen', async (request: NextRequest) => {
  try {
    ...
    if (reportType === 'vat') {
      // Generate VAT report
      const vatReport = generateTaxReport(
        transactions,
...
function calculatePeriodDates(
  ...
  switch (period) {
    case 'monthly':
      ...
      endY = year;
      endM = month - 1;
      endD = lastDayOfMonth(year, month - 1);
    ...
    case 'quarterly':
      ...
      endY = year;
      endM = quarterStart + 2;
      endD = lastDayOfMonth(year, quarterStart + 2);
      nextY = endM === 11 ? year + 1 : year;
      nextM = endM === 11 ? 0 : endM + 1;
      nextD = 1;
      break;
    ...
    default:
      ...
      const nowY = Number(
        nowParts.find((p) => p.type === 'year')?.value ?? new Date().getUTCFullYear(),
      );
...
async function generateComplianceReport(
  transactions: TaxTransactionWithJoins[],
  startDate: string,
  nextStart: string,
) {
