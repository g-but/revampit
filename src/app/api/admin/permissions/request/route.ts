...
export const POST = withAdmin(async (request, session) => {
  try {
    ...
    const invalidSections = sections.filter(
      (s: string) => !validSections.includes(s as AdminSection),
    );
    ...
    if (existing) {
      return apiBadRequest(
        'Du hast bereits eine ausstehende Anfrage für einen oder mehrere dieser Bereiche',
      );
    }
    ...
    try {
      const admins = await db
        ...
        .from(users)
        .where(
          sql`${users.isSuperAdmin} = true OR lower(${users.email}) = ANY(${[...SUPER_ADMIN_EMAILS]}::text[])`,
        );
      ...
      logger.warn('Failed to notify super admins about permission request', {
        error,
