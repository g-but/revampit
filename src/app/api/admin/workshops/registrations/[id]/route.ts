...
export const PUT = withAdmin<{ id: string }>(
  ...
  async (request, session, context) => {
    try {
      ...
      await db.update(workshopRegistrations).set(update).where(eq(workshopRegistrations.id, id));
      ...
      if (
        ...
        (status === WORKSHOP_REGISTRATION_STATUS.CONFIRMED ||
          ...
          status === WORKSHOP_REGISTRATION_STATUS.WAITLIST)
      ) {
        try {
          ...
          logger.error('Workshop status update email failed (rejected)', {
            error: emailError,
            registrationId: id,
          });
        }
