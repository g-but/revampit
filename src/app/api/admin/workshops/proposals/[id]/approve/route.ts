  ...
  async (request, session, context) => {
    ...
    try {
        ...
        if (action === 'approve') {
          ...
          if (proposal.selectedLocationId || proposal.proposedLocation) {
            ...
            if (proposal.proposedDate) {
              const dateStr =
                typeof proposal.proposedDate === 'string'
            ...
            await tx.insert(workshopInstances).values({
              workshopId: workshop.id,
              startDate: proposedDate.toISOString(),
              endDate: endDate.toISOString(),
              location: proposal.selectedLocationId ? null : proposal.proposedLocation,
              locationDetails: proposal.selectedLocationId
                ? JSON.stringify({ location_id: proposal.selectedLocationId })
                : null,
              maxParticipants: proposal.maxParticipants,
      ...
      try {
        ...
        logger.warn('Workshop proposal notification email failed (rejected)', {
          error: emailError,
          proposalId,
          action,
        });
