...
const mockCreateEditSnapshot = vi.fn();
const mockAppendEditHistory = vi.fn();
...
vi.mock('@/db/schema', () => ({
  workshopProposals: {
    ...
    title: 'wp_title',
    status: 'wp_status',
    editHistory: 'wp_editHistory',
    lastEditedBy: 'wp_lastEditedBy',
    lastEditedAt: 'wp_lastEditedAt',
  },
...
vi.mock('@/lib/admin/edit-utils', () => ({
  createEditSnapshot: (...args: unknown[]) => mockCreateEditSnapshot(...args),
  appendEditHistory: (...args: unknown[]) => mockAppendEditHistory(...args),
}));
...
vi.mock('@/components/workshops/types', () => ({}));
...
function makeQueryResult(rows: unknown[]) {
  return {
    ...
    limit: vi.fn((..._args: unknown[]) => Promise.resolve(rows.slice(0, 1))),
  };
}
...
describe('PATCH /api/admin/workshops/proposals/[id] — unauthenticated', () => {
  it('returns 401 when session is null', async () => {
    ...
    const response = await PATCH(
      makeRequest('PATCH', { action: 'edit', fields: { title: 'New' } }),
      makeContext(),
    );
    expect(response.status).toBe(401);
  });
...
describe('PATCH /api/admin/workshops/proposals/[id] — validation', () => {
  it('returns 400 when action is not edit', async () => {
    const response = await PATCH(
      makeRequest('PATCH', { action: 'approve', fields: {} }),
      makeContext(),
    );
  ...
  it('returns 400 when no fields provided', async () => {
    const response = await PATCH(
      makeRequest('PATCH', { action: 'edit', fields: {} }),
      makeContext(),
    );
    expect(response.status).toBe(400);
  });
...
describe('PATCH /api/admin/workshops/proposals/[id] — success', () => {
  it('returns 200 on success', async () => {
    const response = await PATCH(
      makeRequest('PATCH', { action: 'edit', fields: { title: 'Updated Workshop' } }),
      makeContext(),
    );
    expect(response.status).toBe(200);
    const body = await response.json();
