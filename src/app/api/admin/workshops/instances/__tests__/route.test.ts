...
vi.mock('@/db/schema', () => ({
  workshopInstances: {
    ...
    endDate: 'wi_endDate',
    location: 'wi_location',
    instructor: 'wi_instructor',
    maxParticipants: 'wi_maxP',
    notes: 'wi_notes',
    status: 'wi_status',
    createdAt: 'wi_createdAt',
    updatedAt: 'wi_updatedAt',
  },
...
describe('POST /api/admin/workshops/instances — unauthenticated', () => {
  it('returns 401 when session is null', async () => {
    ...
    const response = await POST(
      makeRequest('POST', { workshopId: 'w-1', startDate: '2026-06-01T10:00:00Z' }),
    );
    expect(response.status).toBe(401);
  });
});
...
describe('POST /api/admin/workshops/instances — validation', () => {
  ...
  it('returns 400 when workshop not found', async () => {
    ...
    const response = await POST(
      makeRequest('POST', { workshopId: 'w-bad', startDate: '2026-06-01T10:00:00Z' }),
    );
    expect(response.status).toBe(400);
  });
});
