...
describe('POST /api/admin/workshops/[workshopId]/materials — unauthenticated', () => {
  it('returns 401 when session is null', async () => {
    ...
    const response = await POST(
      makeRequest('POST', { title: 'Slides', url: 'https://example.com', materialType: 'pdf' }),
      makeContext(),
    );
    expect(response.status).toBe(401);
  });
...
describe('POST /api/admin/workshops/[workshopId]/materials — validation', () => {
  it('returns 400 when title is missing', async () => {
    const response = await POST(
      makeRequest('POST', { url: 'https://example.com', materialType: 'pdf' }),
      makeContext(),
    );
  ...
  it('returns 400 when url is missing', async () => {
    const response = await POST(
      makeRequest('POST', { title: 'Slides', materialType: 'pdf' }),
      makeContext(),
    );
    expect(response.status).toBe(400);
  });
  ...
  it('returns 400 when materialType is invalid', async () => {
    const response = await POST(
      makeRequest('POST', { title: 'Slides', url: 'https://example.com', materialType: 'invalid' }),
      makeContext(),
    );
    expect(response.status).toBe(400);
  });
