...
const MOCK_LINE_ITEMS = [{ description: 'Repair service', quantity: 1, unitPrice: 100 }];
...
describe('POST /api/invoices — success', () => {
  ...
  it('allows admin to override userId', async () => {
    ...
    const response = await POST(req);
    expect(response.status).toBe(200);
    expect(mockValues).toHaveBeenCalledWith(expect.objectContaining({ userId: 'other-user-123' }));
  });
});
