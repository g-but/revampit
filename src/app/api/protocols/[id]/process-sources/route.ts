...
export const POST = withAdmin<RouteParams>(
  async (request: NextRequest, session: ValidSession, context) => {
    try {
      ...
      if (
        protocol.status !== PROTOCOL_STATUSES.DRAFT &&
        protocol.status !== PROTOCOL_STATUSES.REVIEW
      ) {
      ...
      if (audioFile) {
        ...
        try {
          ...
          const message =
            error instanceof TranscriptionUnavailableError && error.message
              ? error.message
          ...
          return NextResponse.json(
            {
              success: false,
              error:
                'Keine Sprache erkannt. Bitte deutlicher sprechen oder eine andere Aufnahme verwenden.',
