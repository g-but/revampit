...
export async function GET() {
  ...
  try {
    ...
    const res = await db.execute(
      sql`SELECT count(*)::int as count FROM information_schema.tables WHERE table_name IN (${usersTable}, ${profilesTable})`,
    );
  ...
  return diagnostics.ok
    ? apiSuccess(diagnostics)
    : apiError(new Error('Health check failed'), 'Health check failed', 500);
}
