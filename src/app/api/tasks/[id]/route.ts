 ...
 */
export const GET = withAdmin<RouteParams>(
    ...
    try {
      ...
      const [taskRows, completionRows, flagRows, requestRows] = await Promise.all([
        ...
        db
          ...
          .where(
            and(eq(taskAttentionFlags.taskId, taskId), eq(taskAttentionFlags.isResolved, false)),
          )
        ...
        db
          ...
          .where(
            and(eq(taskRequests.taskId, taskId), eq(taskRequests.status, REQUEST_STATUSES.PENDING)),
          )
 ...
 * Update task
 */
export const PATCH = withAdmin<RouteParams>(
  async (request: NextRequest, session: ValidSession, context) => {
    try {
      const taskId = context?.params?.id;
      ...
      const newAssignee = (data as Record<string, unknown>).assigned_to as
 ...
 */
export const DELETE = withAdmin<RouteParams>(
  async (request: NextRequest, session: ValidSession, context) => {
    try {
      const taskId = context?.params?.id;
