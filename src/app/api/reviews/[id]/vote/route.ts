    ...
    try {
      ...
      const [reviewRows, existingVote] = await Promise.all([
        ...
        db
          ...
          .from(reviewVotes)
          .where(and(eq(reviewVotes.reviewId, reviewId), eq(reviewVotes.voterId, session.user.id))),
      ]);
      ...
      if (existingVote.length > 0) {
        ...
        if (currentVote === voteType) {
          ...
          await db.transaction(async (tx) => {
            await tx
              .delete(reviewVotes)
              .where(
                and(eq(reviewVotes.reviewId, reviewId), eq(reviewVotes.voterId, session.user.id)),
              );
          ...
          await db.transaction(async (tx) => {
            ...
            await tx
              ...
              .set({
                helpfulVotes:
                  voteType === 'helpful' && currentVote === 'unhelpful'
                    ? sql`${reviews.helpfulVotes} + 1`
        ...
        await db.transaction(async (tx) => {
          ...
          await tx
            ...
            .set({
              helpfulVotes:
                voteType === 'helpful' ? sql`${reviews.helpfulVotes} + 1` : reviews.helpfulVotes,
              totalVotes: sql`${reviews.totalVotes} + 1`,
              updatedAt: sql`CURRENT_TIMESTAMP`,
            })
            .where(eq(reviews.id, reviewId));
