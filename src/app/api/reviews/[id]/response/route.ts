...
export const POST = withAuth<{ id: string }>(
  async (request: NextRequest, session: ValidSession, context) => {
    ...
    try {
      ...
      if (
        review.targetType === REVIEW_TARGET_TYPES.REPAIRER &&
        review.repairerUserId !== session.user.id
      ) {
...
export const PUT = withAuth<{ id: string }>(
  async (request: NextRequest, session: ValidSession, context) => {
    ...
    try {
      ...
      const isRepairer =
        response.targetType === REVIEW_TARGET_TYPES.REPAIRER &&
        response.repairerUserId === session.user.id;
...
export const DELETE = withAuth<{ id: string }>(
  async (request: NextRequest, session: ValidSession, context) => {
    ...
    try {
      ...
      const isRepairer =
        responseToDelete.targetType === REVIEW_TARGET_TYPES.REPAIRER &&
        responseToDelete.repairerUserId === session.user.id;
