...
vi.mock('@/db/schema', () => ({
  escrowAccounts: {
    ...
    buyerId: 'ea_buyerId',
    sellerId: 'ea_sellerId',
    totalAmountCents: 'ea_totalAmountCents',
    releasedAmountCents: 'ea_releasedAmountCents',
    currency: 'ea_currency',
    status: 'ea_status',
    auto_release_days: 'ea_autoReleaseDays',
    releaseNotes: 'ea_releaseNotes',
    releasedAt: 'ea_releasedAt',
    updatedAt: 'ea_updatedAt',
  },
  escrowReleases: {
    id: 'er_id',
    escrowAccountId: 'er_escrowAccountId',
    transactionId: 'er_transactionId',
    amountCents: 'er_amountCents',
    releaseType: 'er_releaseType',
    reason: 'er_reason',
    releasedBy: 'er_releasedBy',
  },
  paymentTransactions: {
    id: 'pt_id',
    userId: 'pt_userId',
...
describe('POST /api/payments/escrow/[id] — success', () => {
  it('returns 200 on full release', async () => {
    const { captureTransaction } = await import('@/lib/payments/payrexx-client');
    const req = makePostRequest('escrow-1', { amount: 100, releaseType: 'full' });
    const response = await POST(req, makeContext('escrow-1'));
    expect(response.status).toBe(200);
    const body = await response.json();
