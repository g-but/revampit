...
const ContributionSchema = z.object({
  ...
  message: z
    .string()
    .trim()
    .min(10, 'Bitte beschreibe dein Angebot in mindestens 10 Zeichen')
    .max(4000),
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    sendCustomEmail(
      ...
      .then((r) => {
        if (!r.success) {
          logger.warn('Failed to send project contribution notification (resolved)', {
            error: r.error,
            slug,
          });
        }
      ...
      .catch((err) =>
        logger.warn('Failed to send project contribution notification (rejected)', {
          error: err,
          slug,
        }),
      );
    ...
    sendCustomEmail(email, projectContributionConfirmation(name, project.slug))
      .then((r) => {
        if (!r.success) {
          logger.warn('Failed to send project contribution confirmation (resolved)', {
            error: r.error,
            email,
          });
        }
      ...
      .catch((err) =>
        logger.warn('Failed to send project contribution confirmation (rejected)', {
          error: err,
          email,
        }),
      );
