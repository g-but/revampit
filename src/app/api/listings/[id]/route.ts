...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const is_favorited = session?.user?.id ? await isListingFavorited(session.user.id, id) : false;
  ...
  async (request: NextRequest, session: ValidSession, context?: { params?: { id: string } }) => {
    try {
      ...
      if (data.status !== undefined) {
        const OWNER_MANAGEABLE_STATUSES: string[] = [LISTING_STATUS.DRAFT, LISTING_STATUS.ACTIVE];
        if (
      ...
      await db.transaction(async (tx) => {
        ...
        for (const [key, value] of Object.entries(updateFields)) {
          ...
          if (drizzleKey) {
            ...
            if (drizzleKey === 'priceChf' || drizzleKey === 'shippingCostChf') {
              (setValues as Record<string, unknown>)[drizzleKey] =
        ...
        if (Object.keys(setValues).length > 0) {
          await tx.update(listings).set(setValues).where(eq(listings.id, id));
        }
        ...
        if (specs !== undefined) {
          await tx.delete(listingSpecs).where(eq(listingSpecs.listingId, id));
      ...
      if (
        data.status === LISTING_STATUS.REMOVED ||
        data.status === LISTING_STATUS.SOLD ||
        data.status === LISTING_STATUS.DRAFT
      ) {
        removeListing(id).catch((err) =>
          logger.error('Failed to remove listing from Meilisearch', { error: err, listingId: id }),
        );
      } else {
        ...
        db.select({
          ...
          .then(async (rows) => {
            if (rows[0]) {
              ...
              indexListing({
                ...
                ...meiliSpecs,
              } as MeilisearchDocument).catch((err) =>
          ...
          .catch((err) =>
            logger.error('Failed to fetch listing for Meilisearch index', {
              error: err,
              listingId: id,
            }),
          );
...
export const DELETE = withAuth<{ id: string }>(
  async (request: NextRequest, session: ValidSession, context?: { params?: { id: string } }) => {
    try {
      ...
      await db.update(listings).set({ status: LISTING_STATUS.REMOVED }).where(eq(listings.id, id));
