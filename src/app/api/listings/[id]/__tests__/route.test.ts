...
beforeEach(async () => {
  ...
  const { removeListing, indexListing } = await import('@/lib/search/meilisearch');
  (removeListing as any).mockResolvedValue(undefined);
  (indexListing as any).mockResolvedValue(undefined);
...
describe('GET /api/listings/[id] — not found', () => {
  it('returns 404 when listing not found', async () => {
    ...
    const mockListingLeftJoin: Mock = vi.fn(() => ({
