...
beforeEach(() => {
  ...
  mockSelect.mockReturnValueOnce({ from: mockFrom1 }).mockReturnValueOnce({ from: mockFrom2 });
});
...
describe('GET /api/listings/similar', () => {
  ...
  it('returns 200 with empty array when source listing is not found', async () => {
    ...
    const res = await GET(
      makeRequest('http://localhost:3000/api/listings/similar?listing_id=unknown-id'),
    );
    const body = await res.json();
  ...
  it('returns 200 with similar listings when source exists', async () => {
    ...
    const res = await GET(
      makeRequest('http://localhost:3000/api/listings/similar?listing_id=listing-1'),
    );
    const body = await res.json();
