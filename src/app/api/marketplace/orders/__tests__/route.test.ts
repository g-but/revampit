...
vi.mock('@/lib/payments/payrexx-client', () => ({
  ...
  isPayrexxCheckoutUnavailable: vi.fn(() => false),
...
vi.mock('@/db/schema', () => ({
  ...
  // Referenced by the GET handler's sql`` interpolations; jest returned
  // undefined for unlisted exports, vitest hard-errors on first access.
...
beforeEach(async () => {
  ...
  (emailMod.orderConfirmationBuyer as any).mockReturnValue({});
  (emailMod.newOrderNotificationSeller as any).mockReturnValue({});
