  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (rows.length !== ids.length)
        throw new CartError('Ein Artikel im Warenkorb ist nicht mehr verfügbar');
      for (const r of rows) {
        if (r.status !== LISTING_STATUS.ACTIVE)
          throw new CartError(`«${r.title}» ist nicht mehr verfügbar`);
        if (!r.is_revampit)
          throw new CartError('Nur Revamp-IT-Geräte können über den Warenkorb gekauft werden');
        if (r.seller_id === session.user.id)
          throw new CartError('Du kannst dein eigenes Inserat nicht kaufen');
      }

      const itemsTotalChf =
        Math.round(rows.reduce((s, r) => s + Number(r.price_chf), 0) * 100) / 100;
      const shippingChf =
    ...
    try {
      ...
      try {
        ...
        logger.error('Cart order rollback failed — manual reconciliation required', {
          rollbackError,
          orderId,
        });
      }
