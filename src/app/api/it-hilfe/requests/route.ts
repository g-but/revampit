...
export async function GET(request: NextRequest) {
  try {
    ...
    const conditions: SQL[] = [sql`r.status = ${status}`, sql`r.expires_at > NOW()`];
    ...
    if (matchMySkills) {
      ...
      if (!session?.user?.id) {
        ...
        if (mySkills.length === 0) {
          ...
          conditions.push(
            sql`r.skills_needed IS NOT NULL AND r.skills_needed && ARRAY[${sql.join(
    ...
    if (search && search.trim().length >= 2) {
      ...
      conditions.push(
        sql`(r.title ILIKE ${searchPattern} OR r.description ILIKE ${searchPattern} OR r.device_brand ILIKE ${searchPattern} OR r.device_model ILIKE ${searchPattern})`,
      );
    }
    ...
    const total = rawRequests.length > 0 ? parseInt(rawRequests[0]._total_count || '0', 10) : 0;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isE2ETestAccountEmail(sessionEmail) && !rateLimiters.itHilfeCreate(rateLimitKey)) {
    ...
    if (isNewAnonymousUser) {
      ...
      createPasswordResetToken(requesterEmail, CLAIM_TOKEN_TTL_MS)
        ...
        .catch((err) =>
          logger.error('Anonymous-request claim email failed (rejected)', {
            err,
            requestId,
            email: requesterEmail,
          }),
