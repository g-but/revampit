...
export const POST = withAuth<{ id: string }>(
  async (request: NextRequest, session: ValidSession, context): Promise<NextResponse> => {
    try {
      ...
      if (row.status !== REQUEST_STATUS.COMPLETED) {
        return apiBadRequest('Die Anfrage muss abgeschlossen sein, bevor sie bewertet werden kann');
      }
      ...
      const existingId = await findDuplicateReview(
        session.user.id,
        REVIEW_TARGET_TYPES.IT_HILFE,
        id,
      );
      if (existingId) {
        return apiBadRequest('Diese Anfrage wurde bereits bewertet');
