...
vi.mock('@/db/schema', () => ({
  itHilfeRequests: {
    ...
    canton: 'ihr_canton',
    budgetAmountCents: 'ihr_budgetAmountCents',
    budgetType: 'ihr_budgetType',
    serviceType: 'ihr_serviceType',
    completedAt: 'ihr_completedAt',
    ...
    description: 'ihr_description',
    urgency: 'ihr_urgency',
    postalCode: 'ihr_postalCode',
    city: 'ihr_city',
    imageUrls: 'ihr_imageUrls',
    aiDiagnosis: 'ihr_aiDiagnosis',
    completedBy: 'ihr_completedBy',
  },
  itHilfeOffers: {
    id: 'iho_id',
  ...
  helperProfiles: {
    id: 'hp_id',
    userId: 'hp_userId',
    totalHelpsCompleted: 'hp_totalHelpsCompleted',
  },
...
describe('PUT /api/it-hilfe/requests/[id]', () => {
  ...
  it('returns 403 when user is not owner', async () => {
    mockAuth.mockResolvedValue(MOCK_SESSION);
    mockWhere.mockResolvedValue([
      { requesterId: 'other-user', status: 'open', title: 'X', matchedOfferId: null },
    ]);
    mockFrom.mockReturnValue({ where: mockWhere });
    mockSelect.mockReturnValue({ from: mockFrom });
  ...
  it('returns 400 when validation fails', async () => {
    mockAuth.mockResolvedValue(MOCK_SESSION);
    mockWhere.mockResolvedValue([
      { requesterId: 'user-1', status: 'open', title: 'X', matchedOfferId: null },
    ]);
    mockFrom.mockReturnValue({ where: mockWhere });
    mockSelect.mockReturnValue({ from: mockFrom });
  ...
  it('returns 200 when update succeeds', async () => {
    mockAuth.mockResolvedValue(MOCK_SESSION);
    mockWhere.mockResolvedValue([
      { requesterId: 'user-1', status: 'open', title: 'Old Title', matchedOfferId: null },
    ]);
    mockFrom.mockReturnValue({ where: mockWhere });
    mockSelect.mockReturnValue({ from: mockFrom });
    ...
    const res = await PUT(
      makePutRequest(VALID_UUID, { title: 'New Title' }),
