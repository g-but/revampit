...
export const POST = withAuth<{ id: string }>(
  async (_request: NextRequest, session: ValidSession, context): Promise<NextResponse> => {
    try {
      ...
      if (row.status !== REQUEST_STATUS.MATCHED) {
        return apiBadRequest('Die Anfrage kann nur abgeschlossen werden, wenn sie vergeben ist');
      }
