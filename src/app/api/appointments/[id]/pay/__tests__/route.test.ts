...
beforeEach(async () => {
  ...
  const { processPaymentWithoutInvoice, centsToDisplay } =
    await import('@/lib/payments/payment-flow');
  (processPaymentWithoutInvoice as any).mockResolvedValue({
    paymentUrl: 'https://pay.example.com/abc',
    transactionId: 'txn-1',
    totalAmountCents: 5000,
    currency: 'CHF',
  });
  (centsToDisplay as any).mockImplementation((cents: number) => (cents / 100).toFixed(2));
});
...
describe('POST /api/appointments/[id]/pay — authorization', () => {
  it('returns 401 when user does not own the appointment and is not staff', async () => {
    mockSelect.mockReturnValueOnce(
      makeSelectChain([{ ...MOCK_APPOINTMENT, user_id: 'other-user' }]),
    );
    const req = new NextRequest('http://localhost/api/appointments/appt-1/pay', {
      method: 'POST',
      body: JSON.stringify(DEFAULT_PAY_BODY),
  ...
  it('allows staff to pay on behalf of customer', async () => {
    mockAuth.mockResolvedValueOnce({
      ...MOCK_SESSION,
      user: { ...MOCK_SESSION.user, id: 'admin-1', isStaff: true },
    });
    mockSelect.mockReturnValueOnce(
      makeSelectChain([{ ...MOCK_APPOINTMENT, user_id: 'other-user' }]),
    );
...
describe('POST /api/appointments/[id]/pay — success', () => {
  ...
  it('returns 200 for deposit payment (30%)', async () => {
    ...
    const response = await POST(req);
    expect(response.status).toBe(200);
    const { processPaymentWithoutInvoice } = await import('@/lib/payments/payment-flow');
    const callArgs = (processPaymentWithoutInvoice as any).mock.calls[0][0];
    // 30% of 5000 = 1500
  ...
  it('redirect URLs land on appointment detail page with payment query params', async () => {
    ...
    expect(callArgs.failedRedirectUrl).toMatch(
      /\/dashboard\/appointments\/appt-1\?payment=failed$/,
    );
    expect(callArgs.cancelRedirectUrl).toMatch(
      /\/dashboard\/appointments\/appt-1\?payment=cancelled$/,
    );
  });
});
