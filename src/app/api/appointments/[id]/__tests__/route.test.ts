...
vi.mock('@/db/schema', () => ({
  serviceAppointments: {
    ...
    estimatedDurationHours: 'sa_estimatedDurationHours',
    quotedPriceChf: 'sa_quotedPriceChf',
    quoteApproved: 'sa_quoteApproved',
    quoteApprovedAt: 'sa_quoteApprovedAt',
    diagnosisNotes: 'sa_diagnosisNotes',
    partsNeeded: 'sa_partsNeeded',
    partsOrderedAt: 'sa_partsOrderedAt',
    completedAt: 'sa_completedAt',
    completionNotes: 'sa_completionNotes',
    customerRating: 'sa_customerRating',
    customerReview: 'sa_customerReview',
    reviewedAt: 'sa_reviewedAt',
    lastContactAt: 'sa_lastContactAt',
    messagesCount: 'sa_messagesCount',
...
describe('GET /api/appointments/[id] — authorization', () => {
  it('returns 403 when user is neither customer nor repairer', async () => {
    mockSelect.mockReturnValueOnce(
      makeSelectChain([
        { ...MOCK_APPOINTMENT, user_id: 'other-user', repairer_id: 'other-repairer' },
      ]),
    );
    const req = new NextRequest('http://localhost/api/appointments/appt-1');
    const response = await GET(req, makeContext());
...
describe('GET /api/appointments/[id] — success', () => {
  ...
  it('returns 200 with appointment for repairer', async () => {
    mockAuth.mockResolvedValueOnce({
      ...MOCK_SESSION,
      user: { ...MOCK_SESSION.user, id: 'repairer-1' },
    });
    const req = new NextRequest('http://localhost/api/appointments/appt-1');
    const response = await GET(req, makeContext());
    expect(response.status).toBe(200);
