...
export const PATCH = withAuth<{ id: string }>(
  async (request: NextRequest, session: ValidSession, context?: { params?: { id: string } }) => {
    try {
      ...
      const updated = await executeAppointmentUpdate(
        appointmentId,
        action,
        updateSet,
        appointment.status,
      ...
      if (
        action === 'rate' &&
        updated &&
        appointment.repairerProfileId &&
        actionData.customer_rating
      ) {
        createReview({
          ...
          logger.warn('Failed to mirror appointment rating into reviews', {
            error: err,
            appointmentId,
          }),
        );
