...
vi.mock('@/lib/ai/config/prompts', () => ({
  FORM_AI_REGISTRY: {
    erfassung: {
      auth: 'user',
      systemPrompt: 'You help with erfassung',
      userPromptTemplate: '{text}',
    },
    protocol: {
      auth: 'staff',
      systemPrompt: 'You help with protocols',
      userPromptTemplate: '{text}',
    },
  },
}));
...
describe('POST /api/ai/extract — staff-only access', () => {
  it('returns 403 when non-staff user tries to use a staff-only formType', async () => {
    const req = makeRequest({
      formType: 'protocol',
      text: 'Protocol notes to extract data from here',
    });
    const response = await POST(req);
...
describe('POST /api/ai/extract — success', () => {
  it('returns 200 with extraction result', async () => {
    const req = makeRequest({
      formType: 'erfassung',
      text: 'ThinkPad X1 Carbon by Lenovo, 16GB RAM',
    });
    const response = await POST(req);
    expect(response.status).toBe(200);
    const body = await response.json();
  ...
  it('passes optional mode parameter to registryExtract', async () => {
    ...
    const response = await POST(req);
    expect(response.status).toBe(200);
    expect(mockRegistryExtract).toHaveBeenCalledWith(expect.objectContaining({ mode: 'generate' }));
  });
