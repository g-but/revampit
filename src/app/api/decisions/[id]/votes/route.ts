  ...
  async (request: NextRequest, session: ValidSession, context) => {
    try {
      ...
      if ('error' in result) {
        ...
        if (result.error === 'not_voting_phase')
          return apiBadRequest(ERROR_MESSAGES.VOTE_NOT_IN_VOTING_PHASE);
        if (result.error === 'not_participant')
          return apiBadRequest(ERROR_MESSAGES.VOTE_NOT_PARTICIPANT);
        if (result.error === 'invalid_data') {
          return apiBadRequest(
            'message' in result ? (result.message as string) : ERROR_MESSAGES.VOTE_INVALID_DATA,
          );
        }
      }
