...
export const POST = withAdmin<{ id: string }>(
  async (_request: NextRequest, _session, context): Promise<NextResponse> => {
    try {
    ...
    `);
      const voted = new Set((votedResult.rows as { user_id: string }[]).map((r) => r.user_id));
      ...
      const recipients = userRows.filter((r) => r.email && r.emailNotifications !== false);
