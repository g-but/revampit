...
vi.mock('@/lib/schemas/decisions', () => ({
  ...
  transitionDecisionSchema: {
    safeParse: (body: unknown) => {
      ...
      if (!b?.status)
        return { success: false, error: { issues: [{ message: 'Status erforderlich' }] } };
      return {
        success: true,
        data: { status: b.status, cancelReason: b.cancelReason, outcomeSummary: b.outcomeSummary },
      };
    },
  },
}));
...
describe('POST /api/decisions/[id]/transition — service errors', () => {
  ...
  it('returns 400 when quorum not met', async () => {
    mockTransitionDecision.mockResolvedValueOnce({
      error: 'quorum_not_met',
      message: 'Quorum nicht erreicht',
    });
    const response = await POST(makeRequest({ status: 'closed' }), makeContext());
    expect(response.status).toBe(400);
...
describe('POST /api/decisions/[id]/transition — success', () => {
  ...
  it('notifies staff when transitioning to closed', async () => {
    mockTransitionDecision.mockResolvedValueOnce({
      decision: { ...MOCK_DECISION, status: 'closed' },
    });
  ...
  it('does not notify when transitioning to other status', async () => {
    mockTransitionDecision.mockResolvedValueOnce({
      decision: { ...MOCK_DECISION, status: 'discussion' },
    });
