...
export const GET = withAuth(async (request: NextRequest, session: ValidSession) => {
  try {
    ...
    const result = await db
      ...
      .from(conversations)
      .leftJoin(sql`${users} u1`, sql`${conversations.participant1} = u1.id`)
      .leftJoin(sql`${users} u2`, sql`${conversations.participant2} = u2.id`)
  ...
  try {
    if (!rateLimiters.messageCreate(session.user.id + ':message')) {
      return apiError(
        ...
        'Zu viele Nachrichten. Bitte versuche es später erneut.',
        429,
      );
    }
    ...
    if (recipient_id === session.user.id)
      return apiBadRequest('Nachricht an sich selbst nicht möglich');
    ...
    db.select({ email: users.email, name: users.name })
      ...
      .catch((err) =>
        logger.error('Failed to look up recipient for message email', {
          err,
          recipientId: recipient_id,
        }),
      );
