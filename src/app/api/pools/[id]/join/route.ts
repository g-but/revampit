    ...
    try {
      ...
      const result: JoinResult = await db.transaction(async (tx) => {
        ...
        const pool = lockedRows.rows[0] as
          { id: string; max_members: number; status: string } | undefined;
        ...
        const [existing] = await tx
          ...
          .from(poolMemberships)
          .where(and(eq(poolMemberships.poolId, id), eq(poolMemberships.userId, session.user.id)))
          .limit(1);
      ...
      if (result.kind === 'error') {
        return result.status === 404 ? apiNotFound(result.message) : apiBadRequest(result.message);
      }
