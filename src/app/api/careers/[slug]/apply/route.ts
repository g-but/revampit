...
import { notifyApplicantConfirmation, notifyStaffNewApplication } from '@/lib/hr/notifications';
...
export async function POST(request: NextRequest, context: { params: Promise<Params> }) {
  try {
    ...
    if (
      !posting ||
      !PUBLIC_VACANCY_STATUSES.includes(posting.status as (typeof PUBLIC_VACANCY_STATUSES)[number])
    ) {
      return apiNotFound('Stelle');
    }

    if (
      !vacancyAcceptsApplications(posting.status as import('@/config/hr-vacancies').VacancyStatus)
    ) {
