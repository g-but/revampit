...
jest.mock('@/db/schema', () => ({
  locations: {
    ...
    isApproved: 'loc_isApproved',
    approvalStatus: 'loc_approvalStatus',
    addressLine1: 'loc_addressLine1',
    addressLine2: 'loc_addressLine2',
    postalCode: 'loc_postalCode',
    city: 'loc_city',
    canton: 'loc_canton',
    country: 'loc_country',
    latitude: 'loc_latitude',
    longitude: 'loc_longitude',
    maxCapacity: 'loc_maxCapacity',
    facilities: 'loc_facilities',
    contactName: 'loc_contactName',
    contactPhone: 'loc_contactPhone',
    contactEmail: 'loc_contactEmail',
    createdBy: 'loc_createdBy',
...
const mockLocationSubmissionConfirmation = jest
  .fn()
  .mockReturnValue({ subject: 'test', html: '<p>test</p>' });

jest.mock('@/lib/email', () => ({
  sendCustomEmail: (...args: unknown[]) => mockSendCustomEmail(...args),
  locationSubmissionConfirmation: (...args: unknown[]) =>
    mockLocationSubmissionConfirmation(...args),
...
describe('POST /api/locations — validation', () => {
  ...
  it('returns 400 when duplicate location exists', async () => {
    ...
    mockWhere.mockReturnValueOnce(Promise.resolve([{ id: 'existing-loc' }]));
    const req = makeRequest('POST', VALID_CREATE_BODY);
    const response = await POST(req);
    expect(response.status).toBe(400);
    const body = await response.json();
...
describe('POST /api/locations — success', () => {
  ...
  it('sets createdBy to session user id', async () => {
    ...
    const req = makeRequest('POST', VALID_CREATE_BODY);
    await POST(req);
    expect(mockValues).toHaveBeenCalledWith(expect.objectContaining({ createdBy: 'user-1' }));
  });
});
