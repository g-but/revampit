...
vi.mock('@/db/schema', () => ({
  locations: {
    ...
    approvedBy: 'loc_approvedBy',
    approvedAt: 'loc_approvedAt',
    usageCount: 'loc_usageCount',
    lastUsedAt: 'loc_lastUsedAt',
    updatedAt: 'loc_updatedAt',
  },
  locationApprovals: {
    id: 'la_id',
    locationId: 'la_locationId',
    action: 'la_action',
    reviewedAt: 'la_reviewedAt',
    reviewNotes: 'la_reviewNotes',
  },
  locationBookings: {
    id: 'lb_id',
    locationId: 'lb_locationId',
    bookedBy: 'lb_bookedBy',
    status: 'lb_status',
    startTime: 'lb_startTime',
    endTime: 'lb_endTime',
    title: 'lb_title',
  },
...
beforeEach(() => {
  ...
  mockGroupBy.mockImplementation((..._args: unknown[]) => Promise.resolve([MOCK_LOCATION_ROW]));
...
describe('PUT /api/locations/[id] — validation', () => {
  ...
  it('returns 403 when user does not own location', async () => {
    mockWhere.mockReturnValueOnce(
      Promise.resolve([{ ...MOCK_LOCATION_STUB, createdBy: 'other-user' }]),
    );
    const req = makeRequest('PUT', { name: 'New Name' });
    const response = await PUT(req, makeContext());
  ...
  it('returns 403 when non-admin tries to edit approved location', async () => {
    mockWhere.mockReturnValueOnce(
      Promise.resolve([{ ...MOCK_LOCATION_STUB, approvalStatus: 'approved' }]),
    );
    const req = makeRequest('PUT', { name: 'New Name' });
    const response = await PUT(req, makeContext());
    expect(response.status).toBe(403);
