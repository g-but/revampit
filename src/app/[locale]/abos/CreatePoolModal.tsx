...
export function CreatePoolModal({ onClose, onCreate }: Props) {
  ...
  return (
    ...
    <Modal isOpen onClose={onClose} title={t('modal.title')}>
      {error && (
        <div className="mb-4 p-3 bg-error-50 dark:bg-error-900/20 text-error-700 dark:text-error-400 rounded-xl text-sm">
          {error}
        </div>
      )}
        ...
        <div>
          ...
          {form.monthlyCostChf && form.maxMembers > 0 && (
            <p className="text-xs text-action mt-1">
              {t('modal.perPersonCalc', {
                amount: (Number(form.monthlyCostChf) / form.maxMembers).toFixed(2),
              })}
            </p>
          )}
        </div>
