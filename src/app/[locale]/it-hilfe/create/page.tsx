...
import { DEVICE_CATEGORIES, URGENCY_LEVELS, SERVICE_TYPES, BUDGET_TIERS } from '@/config/it-hilfe';
...
export default function CreatePeerRepairPage() {
  ...
  const { data: preferredTechnicianResult } = useSWR(
    ...
    (url: string) =>
      apiFetch<{
        technician: { id: string; name: string | null; city: string | null; isVerified: boolean };
      }>(url),
  );
    ...
    return (
      <PageShell maxWidth="2xl" py="py-24" className="text-center">
        ...
        <p className="text-text-secondary mb-6 max-w-prose mx-auto">
          Wir haben dir eine E-Mail an <strong>{formData.submitterEmail}</strong> gesendet. Klicke
          auf den Link in der E-Mail, um ein Passwort festzulegen und auf deine Anfrage zuzugreifen.
          Techniker aus der Community werden deine Anfrage sehen, während du dein Konto aktivierst.
        </p>
        <p className="text-xs text-text-tertiary dark:text-text-tertiary mb-6">
          Keine E-Mail erhalten? Überprüfe deinen Spam-Ordner oder fordere auf der Anmeldeseite mit
          &ldquo;Passwort vergessen&rdquo; einen neuen Link an.
        </p>
    ...
    return (
      <PageShell maxWidth="2xl" py="py-24" className="text-center">
        ...
        <p className="text-text-secondary mb-6 max-w-prose mx-auto">
          ...
          <strong>{formData.submitterEmail}</strong> angehängt. Melde dich an, um Angebote zu sehen
          und deine Anfrage zu verwalten.
        </p>
  ...
  return (
    <div className="bg-canvas min-h-screen">
      ...
      <PageShell maxWidth="3xl" py="py-8 sm:py-12">
        ...
        {preferredTechnician && (
          <section
            className="mb-6 rounded-xl border border-action/30 bg-action-muted p-4"
            aria-labelledby="preferred-technician"
          >
            ...
            <div className="mt-2 flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-text-secondary">
                  {t('preferredTechnician.description')}
                </p>
              </div>
              <div className="flex flex-col items-end gap-2">
                <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-action">
                  {preferredTechnician.isVerified
                    ? tTechniker('detail.verified')
                    : tTechniker('detail.community')}
                </span>
                <Link
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <AIFormAssist<AIFormFields>
            ...
            onFieldsFilled={(data, meta) =>
              handleAIFieldsFilled(
                data as Parameters<typeof handleAIFieldsFilled>[0],
                meta as Record<string, AIFieldMetadataEntry>,
              )
            }
          ...
          {status === 'unauthenticated' && (
            <Card className="p-6 border-l-4 border-action">
              ...
              <p className="text-sm text-text-secondary mb-3">
                Damit du Angebote sehen und mit Technikern kommunizieren kannst, brauchen wir deine
                E-Mail. Wir senden dir einen Link, um ein Passwort festzulegen — kein vorheriges
                Konto erforderlich.
              </p>
              <Input
                type="email"
          ...
          {formData.aiDiagnosis && (
            <AIDiagnosisCard
              ...
              deviceInfo={
                [formData.deviceBrand, formData.deviceModel].filter(Boolean).join(' ') || undefined
              }
            />
          )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              {DEVICE_CATEGORIES.map((cat) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-10 h-10 mx-auto mb-2 rounded-lg ${cat.color} flex items-center justify-center`}
                    >
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {BUDGET_TIERS.map((tier) => {
                ...
                const tierCopy = (
                  t.raw as (k: string) => Record<string, { name: string; description: string }>
                )('budgetTiers')[tier.id];
                return (
                  <Button
                    type="button"
            ...
            {BUDGET_TIERS.find((tier) => tier.id === formData.budgetTier)?.requiresAmount && (
              <div className="flex items-center gap-3 mt-4">
                ...
                <span className="text-sm text-text-tertiary">
                  {!formData.maxBudget
                    ? t('budgetFree')
                    : t('budgetUpTo', { amount: formData.maxBudget })}
                </span>
              </div>
            )}
          </Card>
          ...
          <SkillsSection skillsNeeded={formData.skillsNeeded} onSkillToggle={handleSkillToggle} />
