    ...
    <Section id="services" density="spacious" tone="surface" contained={false}>
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className="text-lg text-text-secondary mb-4">{t('subtitle')}</p>
          <div className="text-action font-semibold text-xl mb-8">{t('pricing')}</div>
          <div className="bg-surface-raised/50 rounded-lg p-6 text-left border">
            <Heading level={3} className="text-text-primary mb-2">
              {t('consultationTitle')}
            </Heading>
            <p className="text-text-secondary">{t('consultationText')}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {services.map((service, index) => (
            <div
              key={index}
              className="bg-surface-raised rounded-xl p-8 border hover:border-strong dark:hover:border-white/12 transition-colors duration-300"
            >
