...
export default async function Co2MethodologyPage({ params }: PageProps) {
  ...
  const numbers = CO2_NUMBER_KEYS.map((k) => ORG_NUMBERS_DEFAULTS[k]).filter(Boolean);
    ...
    <div className="min-h-screen">
      ...
      <Section tone="tinted" density="compact">
        <div className="max-w-3xl mx-auto">
          ...
          <div
            className={cn(designPrimitive.surface.card, 'p-6 font-mono text-sm leading-relaxed')}
          >
            <p className="text-text-primary">{t('formula.expression')}</p>
            <p className="mt-3 text-text-secondary">
              {t('formula.exampleLead')} {t('formula.exampleResult')}{' '}
              <strong className="text-text-primary">{t('formula.exampleValue')}</strong>
            </p>
            <p className="mt-3 text-xs text-text-tertiary not-italic">{t('formula.note')}</p>
          </div>
        </div>
      </Section>
      ...
      <Section tone="surface" density="compact">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {numbers.map((n) => (
              <article key={n.key} className={cn(designPrimitive.surface.card, 'p-5')}>
                <div className="flex flex-wrap items-baseline justify-between gap-2 mb-2">
                  <h3 className="text-base font-semibold text-text-primary">{n.label}</h3>
                  <span className="text-xl font-bold text-action tabular-nums">{n.value}</span>
                </div>
                {n.methodology && (
                  <p className="text-sm text-text-secondary mb-2">{n.methodology}</p>
                )}
                {n.calculation && (
                  <p className="text-xs text-text-tertiary font-mono mb-3">{n.calculation}</p>
                )}
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-text-tertiary pt-3 border-t border-subtle">
                  ...
                  <span className="ml-auto">
                    {t('inputs.asOf', { date: n.lastVerified ?? '—' })}
                  </span>
                </div>
              </article>
            ))}
      ...
      <Section tone="tinted" density="compact">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-sm text-text-secondary mb-6">{t('weights.intro')}</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-subtle">
                  {mainCategories.map(([catId, factor]) => {
                    ...
                    return (
                      <tr key={catId}>
                        <td className="px-4 py-3 text-text-primary">{t(`categories.${catId}`)}</td>
                        ...
                        <td className="px-4 py-3 text-xs text-text-tertiary">{factor.ademeItem}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <p className="mt-4 text-sm text-text-secondary">{t('weights.noFactor')}</p>
        </div>
      </Section>
      ...
      <Section tone="surface" density="compact">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-sm text-text-secondary mb-5">{t('sources.intro')}</p>
          <ul className="space-y-2 text-sm">
            {Object.values(CO2_SOURCES).map((source) => (
              <li key={source.url} className="flex gap-3">
                <FileText
                  className="h-4 w-4 shrink-0 mt-0.5 text-text-tertiary"
                  aria-hidden="true"
                />
      ...
      <Section tone="tinted" density="compact">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-sm text-text-secondary mb-3">{t('contribute.intro')}</p>
