...
import { HelpCircle, UserCheck, ShieldCheck, Ban } from 'lucide-react';
  ...
  const columns: AdminTableColumn<HelperRow>[] = [
    {
      ...
      cell: (h) => (
        <div>
          ...
          {h.accepts_gratis && (
            <span className="ml-1 px-1 py-0.5 text-[10px] rounded-sm bg-action-muted text-action">
              {t('badges.gratis')}
            </span>
          )}
          {h.accepts_kulturlegi && (
            <span className="ml-1 px-1 py-0.5 text-[10px] rounded-sm bg-surface-raised text-text-secondary">
              {t('badges.kulturlegi')}
            </span>
          )}
        </div>
      ...
      cell: (h) => (
        <div className="flex flex-wrap gap-1 max-w-xs">
          {(h.skills ?? []).slice(0, 5).map((s) => (
            <SkillTag key={s} skillId={s} />
          ))}
          {(h.skills ?? []).length > 5 && (
            <span className="text-[10px] text-text-tertiary">+{(h.skills ?? []).length - 5}</span>
          )}
        </div>
      ),
    },
      ...
      cell: (h) => (
        <div className="flex items-center gap-1">
          {h.suspended_at ? (
            ...
            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-action-muted text-action">
              {t('badges.verified')}
            </span>
          ) : h.is_active ? (
            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-action-muted text-action">
            ...
            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-surface-raised text-text-tertiary">
              {t('badges.inactive')}
            </span>
          )}
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        <Select
          value={helpFilter.status}
          onChange={(e) => {
            setHelpFilter((f) => ({ ...f, status: e.target.value }));
            setHelpOffset(0);
          }}
          className="w-auto"
        >
          ...
          <option value={HELPER_STATUS.VERIFIED}>
            {HELPER_STATUS_LABELS[HELPER_STATUS.VERIFIED]}
          </option>
          <option value={HELPER_STATUS.SUSPENDED}>
            {HELPER_STATUS_LABELS[HELPER_STATUS.SUSPENDED]}
          </option>
        </Select>
        <Select
          value={helpFilter.canton}
          onChange={(e) => {
            setHelpFilter((f) => ({ ...f, canton: e.target.value }));
            setHelpOffset(0);
          }}
          className="w-auto"
        >
