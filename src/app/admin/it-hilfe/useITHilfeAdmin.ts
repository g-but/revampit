...
export function useITHilfeAdmin() {
  ...
  // Fetch stats
  useEffect(() => {
    apiFetch<Stats>('/api/admin/it-hilfe/stats').then((r) => {
  ...
  const fetchHelpers = useCallback(async () => {
    ...
    const result = await apiFetch<PaginatedResponse<HelperRow>>(
      `/api/admin/it-hilfe/helpers?${params}`,
    );
  ...
  useEffect(() => {
    ...
    async function load() {
      if (tab === 'requests') {
        ...
        if (reqFilter.canton) params.set('canton', reqFilter.canton);
        if (reqFilter.search) params.set('search', reqFilter.search);
        const result = await apiFetch<PaginatedResponse<RequestRow>>(
          `/api/admin/it-hilfe?${params}`,
        );
