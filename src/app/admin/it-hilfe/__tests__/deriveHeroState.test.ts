...
function makeStubTranslator() {
  const fakeT = (key: string, params?: Record<string, unknown>): string => {
    ...
    switch (key) {
      case 'hero.kpis.open':
        return 'Offen';
      case 'hero.kpis.activeTechniker':
        return 'Aktive Techniker';
      case 'hero.kpis.verified':
        return 'Verifiziert';
      case 'hero.kpis.resolutionRate':
        return 'Lösungsrate';
      ...
      case 'hero.urgent.cta':
        return 'Dringende anzeigen';
      ...
      case 'hero.openNoTechniker.cta':
        return 'Bewerbungen prüfen';
      ...
      case 'hero.openWithTechniker.cta':
        return 'Anfragen ansehen';
      case 'hero.noTechniker.headline':
        return 'Noch keine aktiven Techniker';
      case 'hero.noTechniker.sub':
        return 'Aktiviere Bewerbungen, damit das System Anfragen entgegennehmen kann.';
      case 'hero.noTechniker.cta':
        return 'Bewerbungen prüfen';
      case 'hero.healthy.headline':
...
describe('IT-Hilfe deriveHeroState', () => {
  ...
  it('EMPTY when 0 open + 0 active techniker', () => {
    const s = __test__deriveHeroState(makeStats({ activeHelpers: 0 }), onJump, t);
  ...
  it('HEALTHY when 0 open + active techniker exist', () => {
    const s = __test__deriveHeroState(makeStats({ resolutionRate: 92, total: 25 }), onJump, t);
  ...
  it('pluralizes "Anfrage" correctly (1 = singular, >1 = plural)', () => {
    const singular = __test__deriveHeroState(makeStats({ byUrgency: { urgent: 1 } }), onJump, t);
    expect(singular.headline).toContain('1 dringende Anfrage warten');

    const plural = __test__deriveHeroState(makeStats({ byUrgency: { urgent: 5 } }), onJump, t);
  ...
  it('onClick CTA invokes the passed jump function', () => {
    const s = __test__deriveHeroState(makeStats({ byUrgency: { urgent: 1 } }), onJump, t);
