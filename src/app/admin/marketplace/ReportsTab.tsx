...
export function ReportsTab({
  ...
  const columns: AdminTableColumn<ReportRow>[] = [
      ...
      cell: (r) => (
        <>
          ...
          <p className="text-xs text-text-tertiary">
            {t('sellerLabel', { name: r.seller_name || r.seller_email })}
          </p>
        </>
      ),
    },
    {
      ...
      cell: (r) => (
        <>
          ...
          {r.details && (
            <p className="text-xs text-text-tertiary mt-1 max-w-xs truncate">{r.details}</p>
          )}
        </>
      ),
    },
    {
      ...
      cell: (r) => (
        <span className="text-text-secondary">{r.reporter_name || r.reporter_email}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (r) =>
        r.status === REPORT_STATUS.PENDING ? (
          ...
          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-surface-raised text-text-secondary">
            {r.resolution_action ?? t('statusProcessed')}
          </span>
        ),
    },
    {
      ...
      <div className="flex gap-3">
        ...
        >
          <option value={REPORT_STATUS.PENDING}>
            {REPORT_STATUS_LABELS[REPORT_STATUS.PENDING]}
          </option>
          <option value={REPORT_STATUS.REVIEWED}>
            {REPORT_STATUS_LABELS[REPORT_STATUS.REVIEWED]}
          </option>
      ...
      {reports && reports.pagination.total > 50 && (
        <div className="flex items-center justify-between">
          <span className="text-sm text-text-tertiary">
            {t('countLabel', { count: reports.pagination.total })}
          </span>
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              disabled={!reports.pagination.hasMore}
