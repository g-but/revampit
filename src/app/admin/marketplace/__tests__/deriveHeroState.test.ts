...
function makeStubTranslator() {
  const fakeT = (key: string, params?: Record<string, unknown>): string => {
    ...
    switch (key) {
      case 'hero.kpis.active':
        return 'Aktiv';
      case 'hero.kpis.unverified':
        return 'Ungeprüft';
      case 'hero.kpis.openReports':
        return 'Offene Meldungen';
      case 'hero.kpis.revampit':
        return 'RevampIT-Inserate';
      ...
      case 'hero.attention.cta':
        return 'Inserate prüfen';
      case 'hero.healthy.headline':
        return 'Marketplace im grünen Bereich.';
...
describe('Marketplace deriveHeroState', () => {
  it('URGENT wins when openReports > 0 (even if unverified also > 0)', () => {
    const s = __test__deriveHeroState(makeStats({ openReports: 2, unverified: 4 }), onJump, t);
  ...
  it('ATTENTION when openReports === 0 + unverified > 0', () => {
    const s = __test__deriveHeroState(makeStats({ unverified: 7 }), onJump, t);
  ...
  it('pluralizes "Meldung" correctly', () => {
    const singular = __test__deriveHeroState(makeStats({ openReports: 1 }), onJump, t);
    expect(singular.headline).toContain('1 offene Meldung prüfen');

    const plural = __test__deriveHeroState(makeStats({ openReports: 3 }), onJump, t);
  ...
  it('onClick CTA jumps to the right tab', () => {
    const s = __test__deriveHeroState(makeStats({ openReports: 1 }), onJump, t);
    if (s.cta && 'onClick' in s.cta) s.cta.onClick();
    expect(onJump).toHaveBeenCalledWith('reports');
  });

  it('unverified jumps to listings tab', () => {
    const s = __test__deriveHeroState(makeStats({ unverified: 1 }), onJump, t);
    if (s.cta && 'onClick' in s.cta) s.cta.onClick();
