  ...
  const columns: AdminTableColumn<ListingRow>[] = [
    {
      ...
      cell: (l) => (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {l.is_revampit && (
            <span className="px-1.5 py-0.5 text-[10px] rounded-sm bg-action-muted text-action">
              {t('badges.rit')}
            </span>
          )}
    ...
    },
    {
      ...
      cell: (l) => (
        <span className="text-text-primary font-medium">{formatPrice(Number(l.price_chf))}</span>
      ),
    },
    {
      ...
      cell: (l) => (
        <div className="flex items-center gap-1">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => onEdit(l.id, l.admin_notes || '', l.status)}
            className={`p-2 rounded-sm ${adminInteractive.rowHover}`}
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          ...
          {Object.entries(LISTING_STATUS_CONFIG).map(([k, v]) => (
        ...
        <Select
          ...
          onChange={(e) => {
            setFilter((f) => ({ ...f, seller_type: e.target.value }));
            setOffset(0);
          }}
          className="w-auto"
        >
        ...
        <Select
          ...
          onChange={(e) => {
            setFilter((f) => ({ ...f, verified: e.target.value }));
            setOffset(0);
          }}
          className="w-auto"
        >
        ...
        <Select
          ...
          onChange={(e) => {
            setFilter((f) => ({ ...f, reported: e.target.value }));
            setOffset(0);
          }}
          className="w-auto"
        >
      ...
      {listings && listings.items.length === 0 ? (
        <div className="rounded-xl border border-default bg-surface-base p-8 text-center text-text-tertiary">
          {t('empty')}
        </div>
      ) : (
      ...
      {listings && listings.pagination.total > 50 && (
        <div className="flex items-center justify-between">
          <span className="text-sm text-text-tertiary">
            {t('countLabel', { count: listings.pagination.total })}
          </span>
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              disabled={!listings.pagination.hasMore}
