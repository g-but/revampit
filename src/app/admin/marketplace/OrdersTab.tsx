  ...
  const columns: AdminTableColumn<OrderRow>[] = [
    {
      ...
      cell: (o) => (
        <span className="font-mono text-xs text-text-secondary">{o.id.slice(0, 8)}</span>
      ),
    },
    {
      ...
      cell: (o) => (
        <span className="font-medium text-text-primary">
          {o.listing_title || t('cartItems', { count: o.item_count })}
        </span>
      ),
    },
    {
  ...
  return (
    <div className="space-y-4">
      ...
      {orders && orders.items.length === 0 ? (
        <div className="rounded-lg border border-default bg-surface-base p-12 text-center text-text-tertiary">
          {t('empty')}
        </div>
      ) : (
      ...
      {orders && orders.pagination.total > 50 && (
        <div className="flex items-center justify-between">
          <span className="text-sm text-text-tertiary">
            {t('countLabel', { count: orders.pagination.total })}
          </span>
