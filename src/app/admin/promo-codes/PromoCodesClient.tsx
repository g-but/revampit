...
export function PromoCodesClient({ initialCodes }: { initialCodes: PromoRow[] }) {
  ...
  const [form, setForm] = useState({
    ...
    percent: '100',
    amountChf: '',
    scope: PROMO_CODE_SCOPES.ALL as string,
    minOrderChf: '',
    maxRedemptions: '',
    perUserLimit: '',
    validUntil: '',
  ...
  const toggle = async (c: PromoRow) => {
    setBusyId(c.id);
    const res = await apiFetch(`/api/admin/promo-codes/${c.id}`, {
      method: 'PATCH',
  ...
  return (
    <div className="space-y-8">
      ...
      <form
        onSubmit={submit}
        className="rounded-xl border border-subtle bg-surface-base p-4 sm:p-6 space-y-4"
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <label className="block">
            <span className="text-xs font-medium text-text-secondary">
              Mindestbestellwert CHF (optional)
            </span>
            <Input
              type="number"
              min={0}
              step="0.05"
              value={form.minOrderChf}
              onChange={set('minOrderChf')}
              className="mt-1"
            />
          </label>
          <label className="block">
      ...
      <div className="rounded-xl border border-subtle bg-surface-base overflow-hidden">
        <div className="p-4 border-b border-subtle">
          <h2 className="text-base font-medium text-text-primary">
            Ausgestellte Codes ({initialCodes.length})
          </h2>
        </div>
        {initialCodes.length === 0 ? (
          <p className="p-8 text-center text-sm text-text-tertiary">
            Noch keine Codes ausgestellt.
          </p>
        ) : (
              ...
              <li key={c.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-text-tertiary">
                    ...
                    {' · '}
                    {c.redeemedCount}
                    {c.maxRedemptions ? `/${c.maxRedemptions}` : ''} eingelöst
                    {c.perUserLimit ? ` · max ${c.perUserLimit}/Person` : ''}
                    {c.validUntil
                      ? ` · bis ${new Date(c.validUntil).toLocaleDateString('de-CH')}`
                      : ''}
                  </p>
                </div>
                <Button
