...
import { PROTOCOL_WORKFLOW_STEPS, getProtocolWorkflowStep } from '@/lib/protocols/workflow';
import { Plus, FileText, Clock, CheckCircle2, Edit3, AlertTriangle } from 'lucide-react';
...
export default async function ProtocolsAdminPage({
  ...
  let listError = false;
  try {
    const [fetchedStats, fetchedProtocols, fetchedReviewQueue, fetchedTeamMembers] =
      await Promise.all([
    ...
    >
      {listError ? (
        ...
        <Card className="p-12 text-center">
          ...
          <p className="text-text-secondary mb-4">{ADMIN_CONTENT.protocols.emptyDescription}</p>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y">
                  {filteredProtocols.map((protocol) => {
                    const TypeIcon =
                      MEETING_TYPE_ICON_COMPONENTS[protocol.meeting_type as MeetingType];
                    return (
                      <tr key={protocol.id} className={adminTable.tr}>
                        ...
                        <td className="hidden sm:table-cell px-4 py-3 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded-full ${
                              MEETING_TYPE_COLORS[protocol.meeting_type] ||
                        ...
                        <td className="hidden md:table-cell px-4 py-3 whitespace-nowrap">
                          {(() => {
                            ...
                            const workflowIndex =
                              PROTOCOL_WORKFLOW_STEPS.findIndex(
                                (step) => step.id === workflowStep,
                              ) + 1;
                            const workflowLabel =
                              PROTOCOL_WORKFLOW_STEPS.find((step) => step.id === workflowStep)
                                ?.label || 'Workflow';
                            return (
                          ...
                          <div className="inline-flex items-center gap-3">
                            {protocol.action_item_count > 0 && (
                              <span className="hidden sm:inline text-sm text-text-tertiary">
                                {protocol.action_item_count} Aktionen
                              </span>
                            )}
                            <Link
                              href={ROUTES.admin.protocol(protocol.id)}
