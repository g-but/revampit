...
export default async function AnalysePage() {
  ...
  const highPriorityInsights = [...(comparison?.insights || []), ...(yearInsights || [])].filter(
    (i) => i.priority === 'high',
  );

  const positiveInsights = [...(comparison?.insights || []), ...(yearInsights || [])]
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div
                  className={`text-sm mt-2 ${
                    comparison.totalChange.direction === 'up'
                      ? 'text-action'
                      : comparison.totalChange.direction === 'down'
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              <div
                className={`text-sm mt-2 ${
                  latestData.derived.eigenfinanzierungPct.value >= 50
                    ? 'text-action'
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              <div className="text-3xl font-bold mt-1">{missingMetrics.length} ausstehend</div>
                  ...
                  <div key={team} className="text-sm">
                    ...
                    <ul className="mt-1 space-y-1">
                      {metrics.slice(0, 3).map((m) => (
                        <li
                          key={m.id}
                          className="text-warning-700 dark:text-warning-300 flex items-center gap-1"
                        >
            ...
            <div className="mt-4">
              <Link href={ROUTES.admin.analyseKennzahlen}>
                <Button
                  variant="outline"
                  size="sm"
                  className="text-warning-700 dark:text-warning-300 border-warning-300 hover:bg-warning-100 dark:hover:bg-warning-900/30"
                >
              ...
              <ul className="space-y-3">
                {positiveInsights.map((insight) => (
                  <li key={insight.id} className="border-l-4 border-action pl-3">
                    ...
                    <div className="text-sm text-muted-foreground">{insight.description}</div>
                    {insight.implication && (
                      <div className="text-sm text-action mt-1">{insight.implication}</div>
                    )}
                  </li>
                ))}
              </ul>
        ...
        <Link href={ROUTES.admin.analyseFinanzen} className="block">
          <Card className={`${adminInteractive.rowHover} transition-colors`}>
            <CardContent className="pt-4 pb-4 text-center">
              ...
              <div className="text-sm text-muted-foreground">{allData.size} Jahre Daten</div>
            </CardContent>
          </Card>
        </Link>
        ...
        <Link href={ROUTES.admin.analyseWirkung} className="block">
          <Card className={`${adminInteractive.rowHover} transition-colors`}>
            <CardContent className="pt-4 pb-4 text-center">
              ...
              <div className="text-sm text-muted-foreground">Impact-Metriken</div>
            </CardContent>
          </Card>
        </Link>
        <Link href={ROUTES.admin.analyseTransparenz} className="block">
          <Card className={`${adminInteractive.rowHover} transition-colors`}>
            <CardContent className="pt-4 pb-4 text-center">
              ...
              <div className="text-sm text-muted-foreground">Methodologie</div>
            </CardContent>
          </Card>
        </Link>
      </div>
      ...
      <div className="p-4 bg-surface-raised rounded-lg text-sm text-muted-foreground">
        <strong>Datenquellen:</strong> Finanzdaten aus Kivitendo • Letzte Aktualisierung:{' '}
        {latestData?.metadata.importedAt ? formatDateShort(latestData.metadata.importedAt) : 'N/A'}
      </div>
    </div>
  );
}
