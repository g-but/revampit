...
export default async function FinanzenPage() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="p-6 bg-warning-50 dark:bg-warning-900/20 border border-warning-200 dark:border-warning-800 rounded-lg">
          <Heading level={3} className="font-semibold text-warning-800 dark:text-warning-200">
  ...
  const actionableInsights = [...(comparison?.insights || []), ...(insights || [])].filter(
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Detaillierte Finanzübersicht aller Jahre</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <RevenueAreaChart
          ...
          source="Kivitendo"
          sourceDate={
            latestYear?.[1].metadata.importedAt
              ? formatDateShort(latestYear[1].metadata.importedAt)
              : undefined
          }
        />
              ...
              <Card key={year}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{formatCHF(Math.round(yearData.totals.warenverkauf.value))}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Dienstleistungen</span>
                    <span>{formatCHF(Math.round(yearData.totals.dienstleistungen.value))}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Spenden</span>
                    <span>{formatCHF(Math.round(yearData.totals.spenden.value))}</span>
                  </div>
      ...
      <div className="p-4 bg-surface-raised rounded-lg text-sm text-muted-foreground">
        <strong>Datenquelle:</strong> Kivitendo-Export • {yearsData.length} Jahre verfügbar (
