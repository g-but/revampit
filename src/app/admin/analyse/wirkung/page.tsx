  ...
  return (
    <div className="p-4 bg-surface-raised rounded-lg">
      <div className="flex justify-between items-start mb-2">
        ...
        {needsData && <AlertCircle className="w-4 h-4 text-warning-500" />}
      </div>
      <div className={`text-2xl font-bold ${needsData ? 'text-warning-500' : ''}`}>
        {needsData ? '[TBD]' : '—'}
      </div>
      {metric.target && (
        <div className="text-xs text-muted-foreground mt-2">Ziel: {metric.target}</div>
      )}
      {needsData && metric.dataNeeded && (
        <div className="text-xs text-warning-600 mt-2">Benötigt: {metric.dataNeeded}</div>
      )}
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Ökologische und soziale Wirkung von evig</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 bg-action-muted border border-strong rounded-lg">
        <p className="text-sm text-action">
          <strong>Wirkungsbericht:</strong> Diese Seite zeigt die messbare Wirkung von evig. Die
          Daten werden aus verschiedenen Quellen aggregiert und regelmässig aktualisiert.
          {missingMetrics.length > 0 && (
            <span className="block mt-2 text-warning-700 dark:text-warning-300">
              <AlertCircle className="inline w-4 h-4 mr-1" />
              {missingMetrics.length} Metriken benötigen noch Daten für vollständige
              Berichterstattung.
            </span>
          )}
        </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Wie wir unsere Wirkung messen</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <Heading level={4} className="font-medium mb-2">
                CO2-Einsparung
              </Heading>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>evig trägt zu mehreren SDGs bei</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={sdg.number} className="p-4 bg-surface-raised rounded-lg text-center">
      ...
      <div className="p-4 bg-warning-50 dark:bg-warning-900/20 border border-warning-200 dark:border-warning-800 rounded-lg text-sm">
        ...
        <p className="text-warning-700 dark:text-warning-300">
          Um die Wirkung vollständig zu erfassen, benötigen wir regelmässige Daten aus dem
          operativen Betrieb. Die verantwortlichen Teams sollten quartalsweise folgende Daten
          liefern:
        </p>
        <ul className="mt-2 space-y-1 text-warning-700 dark:text-warning-300">
          <li>
            • <strong>Operations:</strong> Anzahl verarbeiteter Geräte, Gewichtsmessungen
          </li>
          <li>
            • <strong>Bildung:</strong> Workshop-Teilnehmer, Schulungsstunden
          </li>
          <li>
            • <strong>HR:</strong> Integrationen, Freiwilligenstunden
          </li>
        </ul>
      </div>
    </div>
