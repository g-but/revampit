    ...
    >
        ...
        {settingsSections.map((section) => {
          const inner = (
            <div className="flex items-start gap-4">
              <div
                className={`w-10 h-10 rounded-lg flex items-center justify-center ${getColorClasses(section.color)}`}
              >
          ...
          return section.href ? (
            ...
            <Card key={section.title} className="p-6 opacity-70" aria-disabled="true">
              {inner}
            </Card>
          );
        })}
      ...
      <div className="p-6 bg-surface-raised border border rounded-xl">
        <p className="text-sm text-text-secondary">
          ...
          Aktuell werden Konfigurationen über Code (<code>src/config/org.ts</code>) und
