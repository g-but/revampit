...
export default async function TeamDetailPage({ params }: PageProps) {
  ...
  const [members, candidates, allTeams, goals, metrics, openTasks, openTaskCount, protocols] =
    await Promise.all([
  ...
  const stateCount = (state: string) =>
    members.filter((m) => (m.work_state ?? 'active') === state).length;
  const available = stateCount(WORK_STATES.ACTIVE);
  const awayStates = (
    [WORK_STATES.ON_LEAVE, WORK_STATES.UNAVAILABLE, WORK_STATES.INACTIVE] as WorkState[]
  )
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          {session.user.isSuperAdmin && (
            <Link
              href={ROUTES.admin.teamEdit(team.slug)}
      ...
      <div className="space-y-5">
        ...
        <Card className="p-5 space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getAccentClasses(team.accent)}`}
            >
              {team.name}
            </span>
            {!team.is_active && (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {team.mail_folders.map((f) => (
                  <span
                    key={f}
                    className="inline-flex items-center gap-1 px-2 py-1 rounded bg-surface-raised text-text-secondary text-xs"
                  >
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {awayStates.map(({ state, count }) => (
                <span
                  key={state}
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${WORK_STATE_COLORS[state]}`}
                >
