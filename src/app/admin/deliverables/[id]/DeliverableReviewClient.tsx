...
import { ExternalLink, Link2, Copy, Check, Bot, Loader2, Send, MessageSquare } from 'lucide-react';
...
export default function DeliverableReviewClient({ deliverable, initialFeedback }: Props) {
  ...
  async function createShareLink() {
    ...
    const res = await apiFetch<{ share_token: string }>(
      `/api/deliverables/${deliverable.id}/share`,
      {
        method: 'POST',
      },
    );
  ...
  async function copyAgentBrief() {
    ...
    const res = await apiFetch<{ prompt: string }>(
      `/api/deliverables/${deliverable.id}/agent-brief`,
    );
  ...
  async function resolve(feedbackId: string, next: string) {
    ...
    if (res.success) {
      setFeedback((prev) =>
        prev.map((f) => (f.id === feedbackId ? { ...f, status: next as FeedbackStatus } : f)),
      );
    }
    setBusy(null);
  }
    ...
    <div className="grid lg:grid-cols-3 gap-6 items-start">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <DeliverableChat
          ...
          suggestions={[
            ...
            'Was gehört zu diesem Liefergegenstand?',
          ]}
        />
                ...
                <li key={f.id} className="border-l-2 border-neutral-200 pl-4">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    {f.kind === FEEDBACK_KINDS.CHANGE_REQUEST && (
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${FEEDBACK_STATUS_COLORS[f.status as FeedbackStatus] ?? ''}`}
                      >
                  ...
                  {f.kind === FEEDBACK_KINDS.CHANGE_REQUEST &&
                    f.status === FEEDBACK_STATUSES.OPEN && (
      ...
      <div className="space-y-4">
        ...
        <Card className="p-5 space-y-3">
          ...
          <p className="text-xs text-text-secondary">
            Fasst die offenen Änderungswünsche + den Quell-Ordner zu einem fertigen Prompt zusammen
            — in Claude Code einfügen, überarbeiten lassen.
          </p>
        </Card>
      </div>
    </div>
