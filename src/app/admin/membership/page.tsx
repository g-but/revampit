...
export default async function MembershipPage() {
  ...
  const [members, totalApplications] = await Promise.all([getMembers(), getApplicationCount()]);
  ...
  const stats: StatItem[] = [
    ...
    { icon: CheckCircle, color: 'blue', label: 'Bezahlt', value: paidCount },
    {
      ...
      label: 'Offen',
      value: unpaidCount,
      valueColor: unpaidCount > 0 ? 'text-secondary-600' : undefined,
    },
  ];
  ...
  const columns: AdminTableColumn<MemberRow>[] = [
          ...
          <>
            <span className="text-text-secondary">
              {MEMBERSHIP_TYPE_LABELS[
                (m.member_type || 'regular') as keyof typeof MEMBERSHIP_TYPE_LABELS
              ] || m.member_type}
            </span>
    ...
    },
    {
      ...
      cell: (m) => (
        <span className="text-text-secondary">
          {m.member_since ? formatDateShort(m.member_since) : '—'}
        </span>
      ),
    },
    {
      ...
      cell: (m) => (
        <span className="text-text-secondary">
          {m.member_paid_until ? formatDateShort(m.member_paid_until) : '—'}
        </span>
      ),
    },
    {
    ...
    {
      header: 'Aktion',
      cell: (m) => <MemberPaymentAction memberId={m.id} paidUntil={m.member_paid_until} />,
    },
  ];
  ...
  if (members.length === 0) {
    return (
      <AdminPageWrapper
        title="Mitglieder"
        description={`${totalApplications} Beitritte total`}
        icon={Users}
        iconColor="green"
      >
