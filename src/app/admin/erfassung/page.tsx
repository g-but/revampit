...
function ErfassungContent() {
  ...
  const handleBulkProductUpdate = useCallback((tempId: string, updates: Partial<BulkProduct>) => {
    setBulkProducts((prev) => prev.map((p) => (p._tempId === tempId ? { ...p, ...updates } : p)));
  }, []);

  const handleBulkProductSelect = useCallback((tempId: string) => {
    setBulkProducts((prev) =>
      prev.map((p) => (p._tempId === tempId ? { ...p, _selected: !p._selected } : p)),
    );
  }, []);
  ...
  const handleBulkSave = useCallback(
    async (action: 'draft' | 'erfassen' | 'publish') => {
      ...
      setBulkProducts((prev) =>
        prev.map((p) => (p._selected ? { ...p, _status: 'processing' } : p)),
      );
