                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {item.required && (
                        <span className="text-xs text-error-500" title={t('requiredMarker')}>
                          *
                        </span>
                      )}
                    </div>
                      ...
                      <div className="mt-1.5 space-y-1">
                        <div className="flex items-center gap-3 flex-wrap">
                          {item.state.completedAt && (
                            <span
                              className={`text-xs ${failed ? 'text-error-600 dark:text-error-400' : 'text-action'}`}
                            >
                              {t('resultAt', {
                        ...
                        {item.state.notes && !notesOpen[item.id] && (
                          <p className="text-xs text-text-tertiary italic pl-0.5">
                            „{item.state.notes}"
                          </p>
                        )}
                      </div>
                    ...
                    {notesOpen[item.id] && (
                      <div className="mt-2 space-y-1.5">
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setNotesText((prev) => ({ ...prev, [item.id]: e.target.value }))
                          }
                          placeholder={
                            pendingFail[item.id]
                              ? t('failNotePlaceholder')
                              : pendingConfirm[item.id]
                                ? t('confirmNotePlaceholder')
                                : t('notePlaceholder')
                          }
                          rows={2}
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            disabled={
                              ...
                              (pendingConfirm[item.id] &&
                                secondPersonOverride[item.id] === true &&
                                (notesText[item.id]?.trim().length ?? 0) < 10)
                            }
                            variant="primary"
                            size="sm"
                          >
