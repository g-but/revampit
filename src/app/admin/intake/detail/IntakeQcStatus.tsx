    ...
    <>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-strong bg-action-muted px-3 py-2.5">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    finalQaItems.forEach((item) =>
                      onSetChecklistResult(item.id, CHECKLIST_RESULTS.PASS),
                    )
                  }
      ...
      {detail.checklist_failed && detail.marketplace_status !== INTAKE_STATUS.PUBLISHED && (
        <div className="border-2 border-error-300 dark:border-error-800 bg-error-50 dark:bg-error-900/20 rounded-lg p-4 space-y-2">
          <Heading
            level={3}
            className="font-medium flex items-center gap-2 text-error-800 dark:text-error-200"
          >
          ...
          {detail.intake_tier && (
            <Button
              ...
              onClick={() => {
                setNewTier(
                  detail.intake_tier === INTAKE_TIERS.PARTS
      ...
      {detail.checklist_complete &&
        (detail.intake_tier === INTAKE_TIERS.REFURBISH ||
          (detail.intake_tier === null && !qcGate)) &&
      ...
      {detail.marketplace_status === INTAKE_STATUS.PUBLISHED &&
        detail.listing_status !== LISTING_STATUS.ACTIVE &&
