  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {items.map((s) => (
        <span
          key={s}
          className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-surface-raised text-text-secondary"
        >
          {s}
        </span>
      ))}
    ...
    <AdminPageWrapper
      ...
      description={
        [
          row?.position,
          row?.employment_type
            ? EMPLOYMENT_TYPE_LABELS[row.employment_type as EmploymentType]
            : null,
        ]
          .filter(Boolean)
          .join(' · ') ||
      ...
      actions={
        <Link
          href={ROUTES.admin.team + '/me/edit'}
      ...
      {saldo && (
        <div className="mb-5 space-y-2">
          ...
          <div className="flex flex-wrap gap-x-4 text-sm">
            ...
            <Link
              href="/admin/zeiterfassung#abwesenheit"
              className="inline-flex min-h-11 items-center font-medium text-action hover:underline"
            >
              {t('saldoRequestAbsence')} →
            </Link>
            <Link
              href={`/admin/team/report/${userId}/${currentMonth}`}
              className="inline-flex min-h-11 items-center font-medium text-action hover:underline"
            >
              {t('reportLink')} →
            </Link>
        ...
        <Card icon={Target} title="Aktueller Fokus">
          {row?.current_focus ? (
            <>
              ...
              {fresh && (
                <p
                  className={`text-xs mt-1 ${fresh.isStale ? 'text-warning-600' : 'text-text-tertiary'}`}
                >
            ...
            <p className="text-sm text-text-tertiary">
              Kein Fokus gesetzt — in «Bearbeiten» ergänzen.
            </p>
          )}
        </Card>
          ...
          <dl className="space-y-2 text-sm">
            ...
            <div>
              ...
              <dd className="text-text-primary">
                {CONTACT_METHOD_LABELS[(row?.preferred_contact ?? 'email') as ContactMethod]}
              </dd>
            </div>
            {row?.phone && (
        ...
        {hasDevelopment && (
          <Card icon={Compass} title="Entwicklung">
            <dl className="space-y-3 text-sm">
              {row?.goals && (
                <div>
                  <dt className="text-xs text-text-tertiary">Ziele</dt>
                  <dd className="text-text-primary whitespace-pre-wrap">{row.goals}</dd>
                </div>
              )}
              {row?.strengths && (
                <div>
                  <dt className="text-xs text-text-tertiary">Stärken</dt>
                  <dd className="text-text-primary whitespace-pre-wrap">{row.strengths}</dd>
                </div>
              )}
              {row?.development_areas && (
                <div>
                  <dt className="text-xs text-text-tertiary">Entwicklungsfelder</dt>
                  <dd className="text-text-primary whitespace-pre-wrap">{row.development_areas}</dd>
                </div>
              )}
            </dl>
          </Card>
