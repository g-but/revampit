...
import { Plus, Tag, Edit, Trash2, CheckCircle, XCircle } from 'lucide-react';
...
export default async function AdminCategoriesPage() {
  ...
  const [stats, categories] = await Promise.all([getCategoryStats(), getCategories()]);
  ...
  const columns: AdminTableColumn<BlogCategory>[] = [
    {
      ...
      cell: (c) => (
        <div>
          ...
          {c.description && (
            <div className="text-sm text-text-tertiary line-clamp-1">{c.description}</div>
          )}
    ...
    },
    {
      header: 'Artikel',
      className: 'whitespace-nowrap',
      cell: (c) => <span className="text-sm text-text-primary">{c.post_count}</span>,
    },
    {
      header: 'Status',
      className: 'whitespace-nowrap',
    ...
    },
    {
      ...
      cell: (c) => (
        <div className="flex items-center gap-2">
          <Link
            href={`/admin/content/categories/${c.id}`}
