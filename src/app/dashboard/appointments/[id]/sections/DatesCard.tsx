...
export function DatesCard({ appointment }: { appointment: AppointmentDetail }) {
  ...
  return (
    <div className={`${CARD_CLASS} space-y-3`}>
      ...
      <DateRow
        icon={<Clock className="w-4 h-4 text-text-muted shrink-0" />}
        label={td('requestedOn')}
        value={formatDateTime(appointment.created_at)}
      />
      {appointment.preferred_date && (
        <DateRow
          icon={<Calendar className="w-4 h-4 text-text-muted shrink-0" />}
          label={td('preferredDate')}
          value={formatDateTime(appointment.preferred_date)}
        />
      )}
      {appointment.confirmed_date && (
        <DateRow
          icon={<CheckCircle className="w-4 h-4 text-action shrink-0" />}
          label={td('confirmedDate')}
          value={formatDateTime(appointment.confirmed_date)}
        />
      )}
      {appointment.completed_at && (
        <DateRow
          icon={<CheckCircle className="w-4 h-4 text-action shrink-0" />}
          label={td('completedAt')}
          value={formatDateShort(appointment.completed_at)}
        />
      )}
    </div>
  );
