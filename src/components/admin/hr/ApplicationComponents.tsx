        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {application.role_track && (
              <span className="text-xs text-text-muted">
                {getRoleTrackLabel(application.role_track)}
              </span>
            )}
          </div>
          ...
          {application.posting_title && (
            <p className="text-sm text-text-muted mt-1">
              {t('postingLabel')}: {application.posting_title}
            </p>
          )}
        </div>
      ...
      {expanded && (
        <div className="border-t border-subtle px-4 sm:px-5 py-4 space-y-4">
          ...
          {application.applicant_phone && (
            <p className="text-sm">
              <span className="text-text-muted">{t('phone')}:</span> {application.applicant_phone}
            </p>
          )}
          <p className="text-sm">
          ...
          {application.admin_notes && (
            <p className="text-sm whitespace-pre-wrap">
              <span className="text-text-muted">{t('notes')}:</span> {application.admin_notes}
            </p>
          )}
          {application.rejection_reason && (
            <p className="text-sm text-error-700">
              <span className="font-medium">{t('rejection')}:</span> {application.rejection_reason}
            </p>
          )}
...
export function ApplicationActionDialog({
  ...
  return (
    <ConfirmDialog
      ...
      message={message}
      details={details}
      confirmLabel={dialog.type === 'hire' ? t('hire') : t('dialog.confirm')}
