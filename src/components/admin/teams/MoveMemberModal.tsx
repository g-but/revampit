...
export default function MoveMemberModal({
  ...
  const targets = useMemo(() => teams.filter((t) => t.id !== fromTeamId), [teams, fromTeamId]);
  ...
  return (
    <Modal
      ...
      title={
        isMove
          ? `${person.name ?? 'Mitglied'} verschieben`
          : `${person.name ?? 'Mitglied'} zuordnen`
      }
    >
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          {isMove && (
            <>
              <span
                className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getAccentClasses(teams.find((t) => t.id === fromTeamId)?.accent)}`}
              >
            ...
            <Select id="move-to" value={toTeamId} onChange={(e) => setToTeamId(e.target.value)}>
              ...
              {targets.map((t) => (
                <option key={t.id} value={t.id}>
                  {t.name}
                  {currentSet.has(t.id) ? ' (bereits Mitglied)' : ''}
                </option>
              ))}
            </Select>
          </div>
        ...
        {selected && (
          <div className="flex items-center gap-2">
            <span
              className={`inline-block w-3 h-3 rounded-full ${getAccentClasses(selected.accent)}`}
              aria-hidden
            />
        ...
        {/* Role */}
        <div>
          <label htmlFor="move-role" className="block text-xs font-medium text-text-secondary mb-1">
            Rolle
          </label>
