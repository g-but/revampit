...
export default function TeamGoalsSection({ teamId, goals }: Props) {
  ...
  async function saveEdit(goalId: string) {
    ...
    const ok = await run(`edit-${goalId}`, () =>
      apiFetch(`/api/admin/teams/${teamId}/goals/${goalId}`, {
        method: 'PATCH',
        body: toBody(draft),
      }),
    );
  ...
  const remove = (goalId: string) =>
    run(`del-${goalId}`, () =>
      apiFetch(`/api/admin/teams/${teamId}/goals/${goalId}`, { method: 'DELETE' }),
    );
          ...
          <li key={g.id}>
            {editingId === g.id ? (
              <GoalDraftForm
                ...
                saving={busy === `edit-${g.id}`}
              />
            ) : (
              <div className="flex items-start gap-2">
                <span
                  className={`mt-0.5 shrink-0 px-2 py-0.5 rounded-full text-[11px] font-medium ${getGoalStatusColor(g.status)}`}
                >
                  {GOAL_STATUS_LABELS[g.status as GoalStatus] ?? g.status}
                </span>
                <div className="min-w-0 flex-1">
                  <p
                    className={`text-sm text-text-primary ${g.status === GOAL_STATUS.DONE ? 'line-through text-text-tertiary' : ''}`}
                  >
                    {g.title}
                    {g.target_label && (
