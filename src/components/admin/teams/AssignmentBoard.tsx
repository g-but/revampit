...
export default function AssignmentBoard({ columns, allTeams }: Props) {
  ...
  const [moving, setMoving] = useState<{ card: TeamMemberRow; fromTeamId: string | null } | null>(
    null,
  );
  ...
  function drop(target: BoardColumn, raw: string) {
    ...
    if (target.teamId === null) {
      ...
      call(p.userId, () =>
        apiFetch(`/api/admin/teams/${p.fromTeamId}/members/${p.membershipId}`, {
          method: 'DELETE',
        }),
      );
      return;
    ...
    call(p.userId, () =>
      apiFetch(`/api/admin/teams/${target.teamId}/members`, {
        ...
        body: {
          user_id: p.userId,
          role: p.role || 'member',
          ...(p.fromTeamId ? { from_team_id: p.fromTeamId } : {}),
        },
      }),
    );
  }
  ...
  return (
    <div className="space-y-3">
          ...
          return (
            <section
              ...
              onDrop={(e) => {
                e.preventDefault();
                setDragOver(null);
                drop(col, e.dataTransfer.getData('application/json'));
              }}
                    ...
                    return (
                      <li
                        ...
                        className={`group card-shell p-2.5 flex items-center gap-2 ${
                          busy
                            ? 'opacity-50'
                            : 'md:cursor-grab active:md:cursor-grabbing hover:border-neutral-300 dark:hover:border-neutral-600'
                        }`}
                      >
                        <GripVertical
                          className="hidden md:block w-4 h-4 text-text-tertiary shrink-0"
                          aria-hidden
                        />
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                            {!isOhne && m.role && (
                              <span
                                className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${getTeamRoleColor(m.role)}`}
                              >
                            ...
                            {m.work_state && m.work_state !== 'active' && (
                              <span
                                className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${WORK_STATE_COLORS[m.work_state as WorkState] ?? ''}`}
                              >
      ...
      <p className="text-xs text-text-tertiary flex items-center gap-1.5 px-1">
        ...
        Am Desktop Karten zwischen Teams ziehen — auf dem Handy tippen und «Verschieben». Ziehen auf
        «Ohne Team» entfernt die Person aus dem Team.
      </p>
      ...
      {moving && (
        <MoveMemberModal
          ...
          person={{
            userId: moving.card.user_id,
            name: moving.card.name || moving.card.email,
            avatarUrl: moving.card.avatar_url,
          }}
          teams={allTeams}
          currentTeamIds={teamsByUser.get(moving.card.user_id) ?? []}
          fromTeamId={moving.fromTeamId}
          fromTeamName={
            moving.fromTeamId ? columns.find((c) => c.teamId === moving.fromTeamId)?.teamName : null
          }
