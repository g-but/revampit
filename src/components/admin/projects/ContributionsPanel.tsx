...
export function ContributionsPanel({ slug, initialContributions, needs }: Props) {
  const t = useTranslations('admin.projects' as never) as (
    k: string,
              ...
              <div key={c.id} className="rounded-lg border p-3 sm:p-4">
                <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      <span
                        className={cn(
                          ...
                          STATUS_BADGE[c.status as ContributionStatus] ?? STATUS_BADGE.new,
                        )}
                      >
                    ...
                    <p className="text-xs text-text-tertiary">
                      ...
                      <span className="font-medium">
                        {need?.title ?? t('contributions.needFallback')}
                      </span>
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-text-tertiary w-full sm:w-auto">
                    <a
                      href={`mailto:${c.email}`}
                      className="inline-flex items-center gap-1 hover:text-action break-all"
                    >
                    ...
                    {c.organization && (
                      <span className="inline-flex items-center gap-1">
                        <Building2 className="h-3.5 w-3.5" /> {c.organization}
                      </span>
                    )}
                  </div>
                </div>
