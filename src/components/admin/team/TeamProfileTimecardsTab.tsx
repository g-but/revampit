...
import { TIMECARD_STATUS_COLORS, TIMECARD_STATUSES, type TimecardStatus } from '@/config/timecards';
    ...
    <div className="space-y-3">
      {saldo && (
        <SaldoStrip
          data={saldo}
          compact
          reportHref={`/admin/team/report/${userId}/${new Date().toISOString().slice(0, 7)}`}
        />
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          disabled={proxyBusy || !proxyMonth}
          onClick={openMonthForUser}
          className="gap-1.5"
        >
          ...
          <ul className="divide-y divide-subtle">
            {rows.map((row) => {
              ...
              return (
                <li
                  key={row.id}
                  className={`px-4 sm:px-5 py-3 ${adminInteractive.rowHoverFaint} transition-colors`}
                >
