...
export function ZeitPensumPanel({ userId, onChanged }: { userId: string; onChanged?: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    if (res.success && res.data) {
      ...
      if (ent) {
        setEntDays(String(Number(ent.days)));
        setEntCarry(String(Number(ent.carryover_days)));
      }
      if (res.data.opening) {
  ...
  return (
    <section className="rounded-xl border border-subtle bg-surface-base">
      ...
      >
        <Eyebrow as="span">{t('zeitPanelTitle')}</Eyebrow>
        <ChevronRight
          className={cn(
      ...
      {open && (
        <div className="space-y-5 border-t border-subtle px-4 py-4 text-sm">
          {error && (
            <p className="rounded-lg border border-error-200 bg-error-50 px-3 py-2 text-error-700">
              {error}
            </p>
          )}
          ...
          <div>
              ...
              <ul className="mb-2 space-y-0.5 text-text-secondary">
                {data.periods.map((p) => (
                  <li key={p.id} className="font-mono tabular-nums">
                    {t('zeitPanelPensumRow', {
                      from: p.valid_from,
                      hours: duration(p.weekly_minutes),
                    })}
                  </li>
                ))}
              </ul>
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                disabled={busy || !periodFrom || !periodHours}
                onClick={() =>
                  post({
                    action: 'period',
                    valid_from: periodFrom,
                    weekly_hours: Number(periodHours),
                  })
                }
                className="gap-1"
              >
            ...
            <div className="flex flex-wrap items-end gap-2">
              <label className="block">
                ...
                <Input
                  ...
                  step="0.5"
                  value={entDays}
                  onChange={(e) => setEntDays(e.target.value)}
                  className="mt-0.5 h-9 w-24"
                />
              </label>
              <label className="block">
                ...
                <Input
                  ...
                  step="0.5"
                  value={entCarry}
                  onChange={(e) => setEntCarry(e.target.value)}
                  className="mt-0.5 h-9 w-24"
                />
              </label>
              <Button
                ...
                variant="outline"
                size="sm"
                disabled={busy || entDays === ''}
                onClick={() =>
                  post({
                    action: 'entitlement',
                    year,
                    days: Number(entDays),
                    carryover_days: Number(entCarry || 0),
                  })
                }
              >
                {t('save')}
          ...
          <div>
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                disabled={busy || openDate === '' || openMinutes === ''}
                onClick={() =>
                  post({
                    action: 'opening',
                    minutes: Math.round(Number(openMinutes) * 60),
                    date: openDate,
                  })
                }
              >
