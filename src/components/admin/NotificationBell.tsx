...
export function NotificationBell() {
  ...
  const handleOpen = () => {
    if (!open && panelRef.current) {
      ...
      setPanelShift(overhang > 0 ? Math.min(overhang, window.innerWidth - margin - rect.right) : 0);
    }
    setOpen((o) => !o);
  ...
  const markOneRead = async (notification: Notification) => {
    if (!notification.is_read) {
      ...
      const result = await apiFetch<void>(`/api/notifications/${notification.id}`, {
        method: 'PATCH',
      });
  ...
  return (
    <div className="relative" ref={panelRef}>
      <Button
        ...
        aria-label={
          unreadCount > 0 ? t('bellAriaWithUnread', { count: unreadCount }) : t('bellAriaLabel')
        }
      >
        ...
        >
          ...
          <div className="max-h-[400px] divide-y overflow-y-auto overflow-x-hidden divide-subtle">
            {error && notifications.length === 0 ? (
              ...
              <div className="py-8 text-center text-sm text-text-tertiary">{t('loading')}</div>
                    ...
                    <div className="flex w-full min-w-0 items-start gap-2">
                      ...
                      <div className={cn('min-w-0 flex-1 break-words', n.is_read && 'pl-4')}>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {href && <span className="text-xs text-action">{t('open')} →</span>}
                        </div>
                      </div>
                    </div>
                  </Button>
