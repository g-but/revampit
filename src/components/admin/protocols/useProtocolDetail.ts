...
export function useProtocolDetail({
  ...
  const handleFinalize = async () => {
    ...
    try {
      const result = await apiFetch<void>(`/api/protocols/${protocol.id}/finalize`, {
        method: 'POST',
      });
  ...
  const handleAddCustomTask = async (
    ...
    try {
      ...
      if (!linkResult.success)
        throw new Error(linkResult.error || 'Fehler beim Erstellen der Aufgabe');
  ...
  const handleCreateAllTasks = async () => {
    ...
    if (failures.length > 0) {
      ...
      setError(
        `${failures.length} Aufgaben konnten nicht erstellt werden. Bitte erneut versuchen.`,
      );
    }
