...
export function useBlogPostForm({ initialData, isEdit = false }: BlogPostFormProps) {
  ...
  const { data, setData, updateField, isSubmitting, error, success, submitCustom, setSuccess } =
    form;
  ...
  const localeIsMachine = useCallback(
    (loc: string) => data.translations[loc]?.isMachine === true,
    [data.translations],
  );

  const setAutoTranslate = useCallback(
    (on: boolean) => updateField('autoTranslate', on),
    [updateField],
  );
