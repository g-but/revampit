...
import {
  getPersonalSectionData,
  TASK_LIMIT,
  SUBMISSION_LIMIT,
} from '@/lib/dashboard/personal-section';
...
export async function PersonalSection({ userId }: PersonalSectionProps) {
  ...
  function contentTypeLabel(type: string | null): string {
    switch (type) {
      case 'blog_post':
        return t('contentTypeLabels.blogPost');
      case 'workshop':
        return t('contentTypeLabels.workshop');
      case 'service':
        return t('contentTypeLabels.service');
      default:
        return t('contentTypeLabels.other');
    }
  }
                  ...
                  <li key={task.id}>
                    ...
                    >
                      {overdue ? (
                        ...
                        <Calendar
                          className="w-4 h-4 text-action shrink-0 mt-0.5"
                          aria-hidden="true"
                        />
                      )}
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-text-primary leading-snug">{task.title}</p>
                        {dueDateText && (
                          <p
                            className={`text-sm mt-0.5 ${overdue ? 'text-error-600 dark:text-error-400' : 'text-text-tertiary'}`}
                          >
                            {dueDateText}
                          </p>
                        )}
            ...
            <ul className="space-y-2" role="list">
              {mySubmissions.map((sub) => (
                <li key={sub.id}>
                  ...
                  >
                    <FileText
                      className="w-4 h-4 text-warning-400 shrink-0 mt-0.5"
                      aria-hidden="true"
                    />
                    <div className="min-w-0 flex-1">
