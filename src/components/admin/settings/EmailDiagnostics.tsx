...
export function EmailDiagnostics() {
  ...
  const [testResult, setTestResult] = useState<{ accepted: boolean; error: string | null } | null>(
    null,
  );
  ...
  const sendTest = async () => {
    setSending(true);
    setTestResult(null);
    const r = await apiFetch<{ accepted: boolean; error: string | null }>(
      ...
      <div className="rounded-xl border border-subtle bg-surface-base p-5">
        ...
        {status.provider === 'smtp' ? (
          <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
            ...
            <div>
              <dt className="text-text-tertiary">Absender</dt>
              <dd className="font-mono text-text-primary">{status.smtp.from}</dd>
            </div>
            <div className="pt-1">
              <Flag ok={status.smtp.userSet} label="Benutzer gesetzt" />
            </div>
            <div className="pt-1">
              <Flag ok={status.smtp.passSet} label="Passwort gesetzt" />
            </div>
          </dl>
        ) : (
          <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
            <div>
              ...
              <dd className="font-mono text-text-primary">{active?.url}</dd>
            </div>
            <div>
              <dt className="text-text-tertiary">Absender</dt>
              <dd className="font-mono text-text-primary">{active?.fromEmail}</dd>
            </div>
            <div className="pt-1">
              <Flag ok={!!active?.userSet} label="Benutzer gesetzt" />
            </div>
            <div className="pt-1">
              <Flag ok={!!active?.passSet} label="Passwort gesetzt" />
            </div>
          </dl>
        )}
      </div>
          ...
          <div className="mt-3 text-sm">
            {testResult.accepted ? (
              <p className="text-success-700 dark:text-success-400">
                Vom Anbieter angenommen. Hinweis: Angenommen ≠ zugestellt — prüfe den Posteingang
                (auch Spam). Bleibt sie aus, liegt es an SPF/DKIM/Reputation der Absenderdomain.
              </p>
            ) : (
              <p className="text-error-600 dark:text-error-400">
                Fehlgeschlagen: {testResult.error}
              </p>
            )}
          </div>
        )}
