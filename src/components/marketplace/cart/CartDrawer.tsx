  ...
  return (
    ...
    >
      ...
      <div
        ref={panelRef}
        tabIndex={-1}
        className="relative flex h-full w-full max-w-sm flex-col border-l border-strong bg-surface-base focus:outline-none"
      >
                ...
                <li key={item.id} className="flex gap-3 py-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <Link
                      ...
                      onClick={closeDrawer}
                      className="line-clamp-2 text-sm font-medium text-text-primary hover:text-action"
                    >
                      {item.title}
                    </Link>
                    <p className="mt-1 font-mono text-sm tabular-nums text-text-secondary">
                      {formatCHF(Number(item.priceChf))}
                    </p>
                  </div>
                  <Button
                    variant="ghost"
