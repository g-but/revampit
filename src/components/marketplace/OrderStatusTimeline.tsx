...
export function OrderStatusTimeline({
  ...
  const stepDate = (key: StepKey): string | null => {
    ...
    switch (key) {
      ...
      case 'paid':
        return timestamps.paidAt ?? null;
      case 'shipped':
        return timestamps.shippedAt ?? null;
      case 'delivered':
        return timestamps.deliveredAt ?? timestamps.completedAt ?? null;
      case 'reviewed':
        return timestamps.reviewedAt ?? null;
    }
  };
            ...
            <div className="flex flex-col items-center shrink-0 min-w-[60px]">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
                  reached ? 'bg-action text-action-text' : 'bg-surface-overlay text-text-muted'
                }`}
              >
                {reached ? <CheckCircle className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
              </div>
              <span
              ...
              {reached && date && (
                <span className="text-[10px] text-text-muted mt-0.5">{formatDateShort(date)}</span>
              )}
            </div>
            {idx < steps.length - 1 && (
              <div
