...
function buildResults(index: SearchIndex | null, query: string, L: GroupLabels): ResultItem[] {
  ...
  for (const m of index.marketplace) {
    ...
    out.push({
      key: `mk:${m.id}`,
      label: m.title,
      sub: m.price_chf != null ? `CHF ${m.price_chf}` : undefined,
  ...
  if (q) {
    out.push({
      key: 'mk-search',
      label: L.marketplaceSearch(query.trim()),
      href: `/marketplace?search=${encodeURIComponent(query.trim())}`,
      group: L.search,
      ...
      >
        <div className="flex items-center gap-3 border-b border px-4 py-3">
          ...
          <Input
            ...
            spellCheck={false}
          />
          <kbd className="shrink-0 rounded-sm bg-surface-raised px-1.5 py-0.5 font-mono text-xs leading-none text-text-tertiary">
            Esc
          </kbd>
        </div>
