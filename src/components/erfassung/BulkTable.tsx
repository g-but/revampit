...
export function BulkTable({
  ...
  const commitEdit = () => {
    if (editingCell) {
      onProductUpdate(editingCell.tempId, {
        [editingCell.field]: editValue,
      } as Partial<BulkProduct>);
      setEditingCell(null);
    }
  };
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface-raised border-b border sticky top-0">
              ...
              {/* Row number */}
              <th className="w-10 px-2 py-3 text-left text-text-secondary font-medium">
                {t('columnNumber')}
              </th>
              {/* Dynamic columns */}
              ...
              {/* Status */}
              <th className="w-16 px-3 py-3 text-center text-text-secondary font-medium">
                {t('columnStatus')}
              </th>
            </tr>
          </thead>
            ...
            {paginatedProducts.map((product, index) => {
              ...
              return (
                <tr
                  ...
                  className={`border-b border-subtle hover:bg-surface-raised transition-colors cursor-pointer ${
                    product._status === 'error'
                      ? 'bg-error-50/50 dark:bg-error-900/10'
                      : product._status === 'saved'
                        ? 'bg-action-muted/50'
                        : ''
                  }`}
                ...
                >
                  ...
                  {BULK_TABLE_COLUMNS.map((col) => {
                    const value =
                      ((product as unknown as Record<string, unknown>)[col.key] as string) || '';
                        ...
                        <td
                          ...
                          onClick={
                            col.editable
                              ? (e) => {
                                  e.stopPropagation();
                                  startEditing(product._tempId, col.key, value);
                                }
                              : undefined
                          }
                        >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-4 py-3 border-t border bg-surface-raised">
          <span className="text-sm text-text-secondary">
            {page * pageSize + 1}-{Math.min((page + 1) * pageSize, products.length)} {t('ofText')}{' '}
