    ...
    <div className="flex flex-wrap items-center justify-between gap-3">
      {view === 'month' ? (
        <>
          ...
          <p className="text-sm text-text-tertiary [@media(pointer:coarse)]:hidden">
            {t('selectHint')}
          </p>
          <p className="hidden text-sm text-text-tertiary [@media(pointer:coarse)]:block">
            {t('selectHintTouch')}
          </p>
        </>
      ) : (
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            size="icon"
            onClick={() => gotoDay(1)}
            disabled={dayIndex < 0 || dayIndex >= tc.visibleDates.length - 1}
