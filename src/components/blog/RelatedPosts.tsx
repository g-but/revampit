  ...
  return (
    <Section density="default" tone="tinted" contained={false}>
      <div className="max-w-[1200px] mx-auto px-6">
        <Heading level={2} className="text-3xl font-bold text-text-primary mb-8">
          {t('relatedArticles')}
        </Heading>
            ...
            <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
              <Card className="overflow-hidden transition-all hover:border-strong">
                ...
                <div className="p-6">
                  <Heading
                    level={3}
                    className="text-xl font-bold text-text-primary mb-2 group-hover:text-action dark:group-hover:text-action transition-colors line-clamp-2"
                  >
                    {post.title}
                  </Heading>
                  {post.excerpt && (
