  ...
  return (
    ...
    >
      ...
      {opt.imageUrl ? (
        ...
        <div
          className={`flex aspect-square w-full items-center justify-center rounded-t-xl text-4xl font-bold ${selected ? 'bg-action-muted text-action' : 'bg-surface-raised text-text-muted'}`}
        >
      ...
      <div className="p-3">
        <p
          className={`truncate text-sm font-medium ${selected ? 'text-action' : 'text-text-primary'}`}
        >
          {opt.label}
        </p>
        {opt.description && (
