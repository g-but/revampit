  ...
  return (
    <div>
      {(resolvedTitle || resolvedDescription) && (
        <div className="mb-3">
          {resolvedTitle && (
            <p className="text-sm font-semibold text-text-primary">{resolvedTitle}</p>
          )}
          {resolvedDescription && (
            <p className="text-sm mt-0.5 text-text-tertiary">{resolvedDescription}</p>
          )}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-2 sm:flex-row sm:items-start">
        ...
        />
        <Button type="submit" disabled={status === 'loading'} variant="primary" size="sm">
          {status === 'loading' ? t('submitting') : t('submit')}
        </Button>
      </form>

      {status === 'error' && (
        <p className="mt-2 text-sm text-error-600 dark:text-error-400">{errorMsg}</p>
      )}
    </div>
  );
}
