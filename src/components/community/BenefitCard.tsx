...
export function BenefitCard({ icon: Icon, title, description, className = '' }: BenefitCardProps) {
  return (
    <div
      className={`card-shell ${responsiveSpacing.cardPadding} hover:border-strong transition-colors duration-300 ${className}`}
    >
      ...
      <Heading
        level={3}
        className={`${responsiveTypography.cardTitle} font-semibold ${responsiveSpacing.mbSmall} text-text-primary`}
      >
        {title}
      </Heading>
...
export function BenefitCardGrid({ children, className = '' }: BenefitCardGridProps) {
  return <div className={`grid grid-cols-1 md:grid-cols-2 gap-8 ${className}`}>{children}</div>;
}
