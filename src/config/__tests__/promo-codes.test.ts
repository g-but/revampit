...
describe('computeDiscount', () => {
  ...
  it('fixed: caps at the order total (never negative)', () => {
    ...
    expect(computeDiscount(code, { ...ctx, totalCents: 4000 })).toEqual({
      ok: true,
      discountCents: 4000,
    });
    expect(computeDiscount(code, { ...ctx, totalCents: 15000 })).toEqual({
      ok: true,
      discountCents: 10000,
    });
  });
  ...
  it('rejects inactive / expired / not-yet-valid', () => {
    expect(computeDiscount({ ...base, isActive: false }, { ...ctx, totalCents: 4000 }).ok).toBe(
      false,
    );
    expect(
      computeDiscount(
        { ...base, validUntil: '2026-07-08T00:00:00Z' },
        { ...ctx, totalCents: 4000 },
      ),
    ).toEqual({ ok: false, reason: 'expired' });
    expect(
      computeDiscount({ ...base, validFrom: '2026-07-10T00:00:00Z' }, { ...ctx, totalCents: 4000 }),
  ...
  it('enforces min order, global and per-user limits', () => {
    expect(computeDiscount({ ...base, minOrderCents: 5000 }, { ...ctx, totalCents: 4000 })).toEqual(
      { ok: false, reason: 'below_min_order' },
    );
    expect(
      computeDiscount(
        { ...base, maxRedemptions: 5, redeemedCount: 5 },
        { ...ctx, totalCents: 4000 },
      ),
    ...
    expect(
      computeDiscount(
        { ...base, perUserLimit: 1 },
        { ...ctx, totalCents: 4000, userRedemptions: 1 },
      ),
  ...
  it('rejects a zero-value discount', () => {
    expect(
      computeDiscount(
        { ...base, type: 'fixed', percent: null, amountCents: 0 },
        { ...ctx, totalCents: 4000 },
      ),
