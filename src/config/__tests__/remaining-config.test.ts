...
import { formatCHF, formatPrice, formatPriceCents } from '../marketplace';
...
describe('getWorkshopRegistrationStatusLabel', () => {
  it('pending → "Ausstehend"', () => {
    expect(getWorkshopRegistrationStatusLabel(WORKSHOP_REGISTRATION_STATUS.PENDING)).toBe(
      'Ausstehend',
    );
  });

  it('confirmed → "Bestätigt"', () => {
    expect(getWorkshopRegistrationStatusLabel(WORKSHOP_REGISTRATION_STATUS.CONFIRMED)).toBe(
      'Bestätigt',
    );
  });

  it('waitlist → "Warteliste"', () => {
    expect(getWorkshopRegistrationStatusLabel(WORKSHOP_REGISTRATION_STATUS.WAITLIST)).toBe(
      'Warteliste',
    );
  });

  it('attended → "Teilgenommen"', () => {
    expect(getWorkshopRegistrationStatusLabel(WORKSHOP_REGISTRATION_STATUS.ATTENDED)).toBe(
      'Teilgenommen',
    );
  });

  it('no_show → "Nicht erschienen"', () => {
    expect(getWorkshopRegistrationStatusLabel(WORKSHOP_REGISTRATION_STATUS.NO_SHOW)).toBe(
      'Nicht erschienen',
    );
  });
...
import { getCategoryStyle, getCategoryBadgeClasses, getCategoryLabel } from '../service-categories';
...
import { getLevelBadgeClass, getCategoryNames } from '../workshops';
