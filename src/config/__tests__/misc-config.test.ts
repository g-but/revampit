...
import { getAppointmentStatusLabel, APPOINTMENT_STATUS } from '../appointment-status';

describe('getAppointmentStatusLabel', () => {
  it('pending_approval → "Genehmigung ausstehend"', () => {
    expect(getAppointmentStatusLabel(APPOINTMENT_STATUS.PENDING_APPROVAL)).toBe(
...
describe('getFieldLabel', () => {
  ...
  it('returns correct label for every workshop field', () => {
    for (const field of Object.keys(WORKSHOP_PROPOSAL_EDITABLE_FIELDS) as Array<
      keyof typeof WORKSHOP_PROPOSAL_EDITABLE_FIELDS
    >) {
  ...
  it('returns correct label for every blog field', () => {
    for (const field of Object.keys(BLOG_SUBMISSION_EDITABLE_FIELDS) as Array<
