...
export async function proxy(request: NextRequest) {
  ...
  const writeLocaleCookie =
    visitedLocaleMatch && visitedLocaleMatch[1] !== request.cookies.get('NEXT_LOCALE')?.value;
  ...
  if (skipIntl) {
    ...
    if (
      shouldIssueCsrfCookie(pathname, method) &&
      !getCsrfFromCookies(request.headers.get('cookie'))
    ) {
