...
describe('useFiltering — initial state', () => {
  ...
  it('honors custom allLabel for non-German UI', () => {
    const { result } = renderHook(() => useFiltering({ items, filters, allLabel: 'All' }));
    expect(result.current.filterState).toEqual({
      category: 'All',
      brand: 'All',
      condition: 'All',
...
describe('useFiltering — getFilterSummary', () => {
  ...
  it('joins multiple active filters with ", "', () => {
    ...
    expect(result.current.getFilterSummary()).toBe('Kategorie: "laptop", Marke: "Apple"');
  });
  ...
  it('preserves configured filter order in the summary', () => {
    ...
    expect(result.current.getFilterSummary()).toBe('Kategorie: "laptop", Zustand: "good"');
  });
});
