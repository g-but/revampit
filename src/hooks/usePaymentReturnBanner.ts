...
export function usePaymentReturnBanner(options: Options = {}) {
  ...
  const [banner, setBanner] = useState<PaymentReturnState | null>(() => {
    ...
    const fromError = paymentErrorToReturnState(
      parsePaymentError(searchParams.get(PAYMENT_ERROR_PARAM)),
    );
    return fromError;
  });
  ...
  return {
    banner,
    dismiss,
    isSuccess: banner === 'success',
    isFailed: banner === 'failed',
    isCancelled: banner === 'cancelled',
  };
}
