...
export const teamProfiles = pgTable(
  'team_profiles',
  {
    ...
    currentFocusUpdatedAt: timestamp('current_focus_updated_at', {
      withTimezone: true,
      mode: 'string',
    }),
...
export const staffPermissionRequests = pgTable(
...
export const activityUpdates = pgTable(
  'activity_updates',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
...
export const userContentSubmissions = pgTable(
...
export const compensationHistory = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    teamProfileId: uuid('team_profile_id')
      .notNull()
      .references(() => teamProfiles.id, { onDelete: 'restrict' }),
    hourlyRateCents: integer('hourly_rate_cents'),
    salaryChf: numeric('salary_chf', { precision: 10, scale: 2 }),
...
export const employmentPeriods = pgTable(
  ...
  (table) => [
    ...
    check(
      'employment_periods_weekly_minutes_range',
      sql`${table.weeklyMinutes} >= 0 AND ${table.weeklyMinutes} <= 6000`,
    ),
  ],
);
...
export const vacationEntitlements = pgTable(
