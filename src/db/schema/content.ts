...
export const presentationComments = pgTable(
  ...
  (table) => [index('presentation_comments_deck_idx').on(table.deckSlug, table.createdAt)],
);
...
export const siteSuggestions = pgTable(
  ...
  (table) => [index('site_suggestions_unresolved_idx').on(table.resolved, table.createdAt)],
);
...
export const blogSubmissions = pgTable(
  'blog_submissions',
  {
    ...
    publishedPostId: uuid('published_post_id').references(() => blogPosts.id, {
      onDelete: 'set null',
    }),
    publishedAt: timestamp('published_at', { withTimezone: true, mode: 'string' }),
