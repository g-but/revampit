...
export const serviceAppointments = pgTable(
  'service_appointments',
  {
    ...
    repairerProfileId: uuid('repairer_profile_id').references(() => repairerProfiles.id, {
      onDelete: 'set null',
    }),
...
export const repairerProfiles = pgTable(
  'technician_profiles',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
    ...
    serviceDeliveryTypes: text('service_delivery_types')
      .array()
      .default(sql`'{flexible}'`),
...
export const repairerServices = pgTable(
  'technician_services',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    repairerId: uuid('repairer_id')
      .notNull()
      .references(() => repairerProfiles.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    ...
    uniqueIndex('repairer_services_repairer_category_name_unique').on(
      table.repairerId,
...
export const repairerAvailability = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('repairer_availability_repairer_date_start_unique').on(
      table.repairerId,
      table.date,
      table.startTime,
    ),
...
export const repairerReviews = pgTable(
  'technician_reviews',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    repairerId: uuid('repairer_id')
    ...
    repairerResponseDate: timestamp('repairer_response_date', {
      withTimezone: true,
      mode: 'string',
    }),
  ...
  (table) => [
    ...
    uniqueIndex('repairer_reviews_repairer_customer_appointment_unique').on(
      table.repairerId,
      table.customerId,
      table.appointmentId,
    ),
