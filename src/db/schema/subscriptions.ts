...
export const subscriptionPools = pgTable(
  'subscription_pools',
  {
    ...
    costPerMemberChf: decimal('cost_per_member_chf', { precision: 10, scale: 2 }).generatedAlwaysAs(
      sql`monthly_cost_chf / max_members`,
    ),
  ...
  (table) => [index('idx_subscription_pools_status').on(table.status)],
);
...
export const poolMemberships = pgTable(
  'pool_memberships',
  {
    ...
    poolId: uuid('pool_id')
      .notNull()
      .references(() => subscriptionPools.id, { onDelete: 'cascade' }),
    userId: uuid('user_id')
      .notNull()
      .references(() => users.id),
...
export const poolVotes = pgTable(
  ...
  (table) => [
    uniqueIndex('pool_votes_pool_id_voter_id_vote_type_key').on(
      table.poolId,
      table.voterId,
      table.voteType,
    ),
