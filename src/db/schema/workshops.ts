...
export const workshopInstances = pgTable(
  'workshop_instances',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const workshopRegistrations = pgTable(
  'workshop_registrations',
  {
    ...
    workshopInstanceId: uuid('workshop_instance_id')
      .notNull()
      .references(() => workshopInstances.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    ...
    uniqueIndex('workshop_registrations_user_instance_unique').on(
      table.userId,
      table.workshopInstanceId,
    ),
  ],
);
...
export const workshopProposals = pgTable(
  'workshop_proposals',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: uuid('user_id')
...
export const workshopMaterials = pgTable(
  'workshop_materials',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    workshopId: uuid('workshop_id')
      .notNull()
      .references(() => workshops.id, { onDelete: 'cascade' }),
    instanceId: uuid('instance_id').references(() => workshopInstances.id, {
      onDelete: 'set null',
    }),
