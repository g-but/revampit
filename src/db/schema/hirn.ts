...
export const hirnDocuments = pgTable(
  'hirn_documents',
  {
...
export const hirnChunks = pgTable(
  'hirn_chunks',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    documentId: uuid('document_id')
      .notNull()
      .references(() => hirnDocuments.id, { onDelete: 'cascade' }),
  ...
  (table) => [index('idx_hirn_chunks_document_id').on(table.documentId)],
);
...
export const hirnChatHistory = pgTable(
  'hirn_chat_history',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const hirnProviderSettings = pgTable(
  ...
  (table) => [
    uniqueIndex('hirn_provider_settings_scope_user_id_provider_key').on(
      table.scope,
      table.userId,
      table.provider,
    ),
