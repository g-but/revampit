...
export type ReviewResult<Row> =
  { ok: true; row: Row; from: string; to: string | null } | ReviewFailure;
...
> {
  ...
  applyInTxn?: (
    tx: Tx,
    row: Row,
    ctx: { from: string; to: string | null; action: A },
  ) => Promise<void>;
  /**
  ...
  emit?: (
    ...
    | null
    | Promise<WorkflowEvent | readonly WorkflowEvent[] | null>;
  db?: DbOrTx;
}
...
>(opts: RunReviewTransitionOptions<Row, A>): Promise<ReviewResult<Row>> {
  ...
  if (opts.emit) {
    try {
      const events = await opts.emit(done.row, {
        from: done.from,
        to: done.to,
        action: opts.action,
      });
      ...
      logger.warn('review-workflow emit failed', {
        table: opts.target.table,
        id: opts.id,
        action: opts.action,
        error,
      });
