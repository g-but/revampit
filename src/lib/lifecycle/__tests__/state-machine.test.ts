...
const ORDER_TABLE: TransitionTable<
  'paid' | 'shipped' | 'delivered' | 'completed' | 'cancelled' | 'pending_payment',
...
describe('resolveTransition — basic decisions', () => {
  it('unknown_action when no row matches the action', () => {
    expect(
      resolveTransition(ORDER_TABLE, { from: 'paid', action: 'completed', role: 'buyer' }),
    ).toEqual({ ok: false, reason: 'wrong_state' }); // completed exists but from!=delivered
    expect(
      resolveTransition(ORDER_TABLE, { from: 'paid', action: 'delivered', role: 'buyer' }),
    ).toEqual({ ok: false, reason: 'wrong_role' }); // delivered is seller-only
  });

  it('wrong_role when the actor cannot perform the action', () => {
    expect(
      resolveTransition(ORDER_TABLE, { from: 'paid', action: 'shipped', role: 'buyer' }),
    ).toEqual({ ok: false, reason: 'wrong_role' });
  });

  it('wrong_state when role is allowed but the current status is wrong', () => {
    expect(
      resolveTransition(ORDER_TABLE, { from: 'paid', action: 'delivered', role: 'seller' }),
    ).toEqual({ ok: false, reason: 'wrong_state' });
  });

  it('ok with the target status when role and from both match', () => {
    expect(
      resolveTransition(ORDER_TABLE, { from: 'paid', action: 'shipped', role: 'seller' }),
    ).toEqual({ ok: true, to: 'shipped' });
    expect(
      resolveTransition(ORDER_TABLE, { from: 'delivered', action: 'completed', role: 'buyer' }),
...
describe('resolveTransition — role variants', () => {
  ...
  it('picks the matching row when an action has multiple rows (cancelled from paid vs pending_payment)', () => {
    expect(
      resolveTransition(ORDER_TABLE, { from: 'paid', action: 'cancelled', role: 'buyer' }),
    ).toEqual({ ok: true, to: 'cancelled' });
    ...
    expect(
      resolveTransition(ORDER_TABLE, { from: 'paid', action: 'cancelled', role: 'seller' }),
    ).toEqual({ ok: false, reason: 'wrong_state' });
  });
});

describe('resolveTransition — appointment-shaped (role omitted, to omitted, role array)', () => {
  const APPT_TABLE: TransitionTable<
    'requested' | 'completed' | 'cancelled' | 'accepted',
    'rate' | 'cancel' | 'unknownish',
    'customer' | 'repairer'
  > = [
  ...
  it('returns to: null for an action with no target status', () => {
    expect(
      resolveTransition(APPT_TABLE, { from: 'completed', action: 'rate', role: 'customer' }),
    ).toEqual({ ok: true, to: null });
  });

  it('honours an array of allowed from-states', () => {
    expect(
      resolveTransition(APPT_TABLE, { from: 'accepted', action: 'cancel', role: 'repairer' }),
    ).toEqual({ ok: true, to: 'cancelled' });
    expect(
      resolveTransition(APPT_TABLE, { from: 'completed', action: 'cancel', role: 'customer' }),
    ).toEqual({ ok: false, reason: 'wrong_state' });
  });

  it('unknown_action for an action with no rows', () => {
    expect(
      resolveTransition(APPT_TABLE, { from: 'completed', action: 'unknownish', role: 'customer' }),
    ).toEqual({ ok: false, reason: 'unknown_action' });
  });

  it('null actor role fails role-gated rows', () => {
    expect(
      resolveTransition(APPT_TABLE, { from: 'completed', action: 'rate', role: null }),
    ).toEqual({ ok: false, reason: 'wrong_role' });
  });
});
...
describe('canTransition', () => {
  it('is true only when resolveTransition is ok', () => {
    expect(canTransition(ORDER_TABLE, { from: 'paid', action: 'shipped', role: 'seller' })).toBe(
      true,
    );
    expect(canTransition(ORDER_TABLE, { from: 'paid', action: 'shipped', role: 'buyer' })).toBe(
      false,
    );
  });
});
