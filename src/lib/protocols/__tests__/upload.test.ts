...
describe('classifyFile', () => {
  ...
  it('classifies by MIME when extension is unknown', () => {
    expect(classifyFile({ name: 'unknown.bin', type: 'audio/wav' })).toBe(
      PROTOCOL_UPLOAD_KIND.AUDIO,
    );
    expect(classifyFile({ name: 'unknown.bin', type: 'text/plain' })).toBe(
      PROTOCOL_UPLOAD_KIND.TEXT,
    );
    expect(classifyFile({ name: 'unknown.bin', type: 'application/json' })).toBe(
      PROTOCOL_UPLOAD_KIND.TEXT,
    );
  });
...
describe('validateTextUpload', () => {
  ...
  it('rejects file larger than the cap', () => {
    expect(
      validateTextUpload({ name: 'big.txt', size: TEXT_UPLOAD_LIMITS.maxSizeBytes + 1 }),
    ).toMatch(/zu gross/);
  });

  it('accepts file exactly at the cap', () => {
    expect(
      validateTextUpload({ name: 'exact.txt', size: TEXT_UPLOAD_LIMITS.maxSizeBytes }),
    ).toBeNull();
  });
});
