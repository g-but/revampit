...
export function getProtocolReviewCounts(
  ...
  return {
    ...
    closedDecisionsWithoutTask: decisions.filter((item) => {
      ...
      return (
        linkedDecision.status === DECISION_STATUS.CLOSED &&
        linkedDecision.outcomePassed !== false &&
        !linkedDecision.linkedTaskId
      );
    }).length,
    infos: actionItems.filter((item) => item.item_type === 'info').length,
    unresolvedAssignees: tasks.filter((item) => item.assigned_to_name && !item.assigned_to_id)
      .length,
...
export function getProtocolReviewChecklist(input: {
  ...
  const counts = getProtocolReviewCounts(input.notes, input.actionLinks, input.protocolDecisions);
  ...
  return [
    {
      ...
      description: input.hasRawInput
        ? 'Transkript, Audio oder Notizen sind am Protokoll gespeichert.'
        : 'Es fehlt noch verwertbarer Input.',
