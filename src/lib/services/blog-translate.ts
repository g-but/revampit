...
const RETRY_BASE_MS = process.env.VITEST_WORKER_ID ? 0 : 2000;
...
async function translateMeta(
  ...
  if (!result?.text) return null;
  const parsed = extractJson(result.text, /\{[\s\S]*\}/) as {
    title?: string;
    excerpt?: string;
  } | null;
...
export async function fillMissingTranslations(
  ...
  const inScope = (l: string) =>
    TRANSLATABLE_LOCALES.includes(l as (typeof TRANSLATABLE_LOCALES)[number]);
  const targets = (
    requested?.filter(inScope) ?? TRANSLATABLE_LOCALES.filter((l) => !existingByLocale.has(l))
