...
import { parseWeeklySchedule, weekdayIdFromDate, getScheduleDayMinutes } from '@/lib/team/schedule';
import { getOrCreateTimecardForUser, saveTimecardDraft } from '@/lib/services/timecards';
...
export async function materializeLeaveEntries(params: {
  ...
  for (const [month, dates] of datesByMonth) {
    try {
      const card = await getOrCreateTimecardForUser(userId, {
        period_type: 'month',
      ...
      const additions: TimecardEntryInput[] = dates
        ...
        .map((date) => {
          ...
          const minutes = paid
            ? hasSchedule && day.enabled
              ? getScheduleDayMinutes(day)
              : 8 * 60
            : 0;
          return {
            work_date: date,
