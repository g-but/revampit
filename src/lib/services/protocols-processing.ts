...
import {
  processProtocolTranscript,
  processProtocolNotes,
  processTaskList,
} from '@/lib/ai/protocol-processing';
...
async function getTeamRoster(): Promise<{ promptString: string; userRows: UserRow[] }> {
  try {
    const result = await db.execute(
      sql`SELECT id, name FROM ${sql.raw(uTable)} WHERE name IS NOT NULL`,
    );
  ...
  try {
    ...
    if (protocolResult.rows.length === 0) {
      ...
      return {
        ...
        retryable: false,
        error: ERROR_MESSAGES.PROTOCOL_NOT_FOUND,
      };
    }
...
export async function recoverStaleProtocolProcessing(protocolId: string): Promise<boolean> {
  ...
  if (recovered) {
    logger.warn('Recovered protocol stuck in processing', {
      protocolId,
...
export async function processNotes(
  ...
  if (!aiResult) {
    ...
    return {
      success: false,
      error:
        'KI-Verarbeitung der Notizen fehlgeschlagen. Kein Provider erreichbar. Details im Server-Log.',
    };
  }
...
export async function importTasks(
  ...
  try {
    ...
    if (!aiResult) {
      return {
        success: false,
        error:
          'KI-Verarbeitung der Aufgabenliste fehlgeschlagen. Kein Provider erreichbar. Details im Server-Log.',
      };
    }
  ...
  const existingRow = existingResult.rows[0] as unknown as
    { structured_notes: StructuredNotes | null } | undefined;
