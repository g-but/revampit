...
describe('reviewTimecard workflow wiring', () => {
  it('passes review transitions, guards and notification/activity event builder to the core', async () => {
    await expect(
      reviewTimecard('reviewer-1', 'card-1', {
    ...
    expect(opts.transitions).toEqual(
    ...
    expect(opts.guards.map((guard: { code: string }) => guard.code)).toEqual([
      'self_review',
      'payroll_locked',
    ]);
  ...
  it('maps core failures to the stable thrown error codes used by routes and bulk review', async () => {
    await expect(
      reviewTimecard('reviewer-1', 'missing', { status: TIMECARD_STATUSES.APPROVED }),
    ).rejects.toThrow('timecard_not_found');
    ...
    await expect(
      reviewTimecard('owner-1', 'card-1', { status: TIMECARD_STATUSES.APPROVED }),
    ).rejects.toThrow('timecard_self_review');

    mockRunReviewTransition.mockResolvedValueOnce({
      ok: false,
      code: 'guard_failed',
      guard: 'payroll_locked',
    });
    await expect(
      reviewTimecard('reviewer-1', 'card-1', { status: TIMECARD_STATUSES.APPROVED }),
  ...
  it('keeps the self_review guard by default but drops it when allowSelfReview is set', async () => {
    ...
    const defaultGuards = mockRunReviewTransition.mock.calls[0][0].guards.map(
      (g: { code: string }) => g.code,
    );
    ...
    await expect(
      reviewTimecard(
        'reviewer-1',
        'card-1',
        { status: TIMECARD_STATUSES.APPROVED },
        { allowSelfReview: true },
      ),
    ).rejects.toThrow('timecard_not_found');
    const exemptGuards = mockRunReviewTransition.mock.calls[0][0].guards.map(
      (g: { code: string }) => g.code,
    );
...
describe('reopenTimecard workflow wiring', () => {
  it('clears review/submission metadata and emits notification plus activity for non-owner reopen', async () => {
    ...
    expect(opts.write.reopen).toEqual(
      expect.objectContaining({
        reviewer: 'clear',
        reason: 'clear',
      }),
