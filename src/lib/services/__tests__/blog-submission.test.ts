...
describe('rejectSubmission', () => {
  ...
  it('email failure does NOT propagate', async () => {
    ...
    await expect(rejectSubmission(makeSubmission(), REVIEWER, 'reason')).resolves.toMatchObject({
      status: APPROVAL_STATUS.REJECTED,
    });
  });
});
...
describe('requestChanges', () => {
  it('returns requires_changes status and Swiss-German message', async () => {
    const result = await requestChanges(
      makeSubmission(),
      REVIEWER,
      'Bitte Abschnitt 2 überarbeiten',
    );
  ...
  it('email failure does NOT propagate', async () => {
    ...
    await expect(requestChanges(makeSubmission(), REVIEWER, 'notes')).resolves.toMatchObject({
      status: APPROVAL_STATUS.REQUIRES_CHANGES,
    });
  });
});
...
describe('editSubmission — guards', () => {
  it('throws EditNotAllowedError when status is APPROVED (not PENDING)', async () => {
    ...
    await expect(editSubmission(approved, REVIEWER, 'Editor', { title: 'New' })).rejects.toThrow(
      EditNotAllowedError,
    );
  });
  ...
  it('throws EditNotAllowedError when status is PUBLISHED', async () => {
    ...
    await expect(editSubmission(published, REVIEWER, 'Editor', { title: 'New' })).rejects.toThrow(
      EditNotAllowedError,
    );
  });

  it('throws NoFieldsError when fields object is empty', async () => {
    await expect(editSubmission(makeSubmission(), REVIEWER, 'Editor', {})).rejects.toThrow(
      NoFieldsError,
    );
  });
...
describe('editSubmission — happy path', () => {
  ...
  it('returns the updated submission and success message', async () => {
    const result = await editSubmission(makeSubmission(), REVIEWER, 'Editor', {
      title: 'Neuer Titel',
    });
