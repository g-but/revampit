...
vi.mock('@/db/schema/misc', () => {
  ...
  // misc is a wide barrel; jest returned undefined for unlisted exports,
  // vitest hard-errors — the Proxy stubs whatever the module graph touches.
  return new Proxy(explicit, {
    has: () => true,
    get: (t, p) =>
      p in t ? t[p as string] : typeof p === 'string' && p !== 'then' ? {} : undefined,
  });
});
...
describe('getDecisions', () => {
  ...
  it('sets hasUserVoted=false when user has not voted', async () => {
    const row = makeDbRow({ id: 'dec-1' });
    mockDbExecute.mockResolvedValueOnce({ rows: [row] }).mockResolvedValueOnce({ rows: [] }); // no votes for this user
  ...
  it('reads total from _total_count on first row', async () => {
    ...
    mockDbExecute.mockResolvedValueOnce({ rows: [row] }).mockResolvedValueOnce({ rows: [] });
    ...
    await createDecision(
      {
        ...BASE_DATA,
        options: [
          { label: 'Ja', imageUrl: undefined },
          { label: 'Nein', imageUrl: undefined },
        ],
      },
      CREATOR,
    );
  ...
  it('returns public data when status is DISCUSSION', async () => {
    mockDbExecute.mockResolvedValueOnce({
      rows: [
        {
          id: 'dec-2',
          status: DECISION_STATUS.DISCUSSION,
          title: 'X',
          description: 'Y',
          background: null,
