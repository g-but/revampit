import { buildFollowUpTaskPayload, getDecisionOutcomePassed } from '../protocol-decision-tasks';

describe('getDecisionOutcomePassed', () => {
  it('returns true for passed thumbs votes', () => {
    expect(
      getDecisionOutcomePassed({ passed: true, method: 'thumbs_up_down' }, 'thumbs_up_down'),
    ).toBe(true);
  });

  it('returns false for rejected thumbs votes', () => {
    expect(
      getDecisionOutcomePassed({ passed: false, method: 'thumbs_up_down' }, 'thumbs_up_down'),
    ).toBe(false);
  });
