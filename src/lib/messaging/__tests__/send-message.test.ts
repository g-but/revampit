...
const mockTxSelect = vi.fn(() => makeChain([]));
const mockTxInsert = vi.fn(() => makeChain([]));
const mockTxUpdate = vi.fn(() => makeChain([]));

const mockTx = {
  select: (...args: unknown[]) => mockTxSelect.apply(null, args as never),
  insert: (...args: unknown[]) => mockTxInsert.apply(null, args as never),
  update: (...args: unknown[]) => mockTxUpdate.apply(null, args as never),
};
