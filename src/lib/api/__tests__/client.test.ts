...
describe('apiFetch', () => {
  ...
  it('surfaces the server error message when success=false', async () => {
    mockFetch.mockResolvedValueOnce(
      jsonResponse({ success: false, error: 'Validierung fehlgeschlagen' }, 400),
    );
  ...
  it('treats success=false on a 200 as an error too', async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      jsonResponse({ success: false, error: 'Nicht stimmberechtigt' }, 200),
    );
  ...
  it('passes a machine-readable error code through to the caller', async () => {
    mockFetch.mockResolvedValueOnce(
      jsonResponse(
        { success: false, error: 'Bitte melde dich an.', code: 'vote_email_registered' },
        400,
      ),
    );
