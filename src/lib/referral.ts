...
export async function getOrCreateReferralCode(userId: string) {
  ...
  const [created] = await db.insert(referralCodes).values({ userId, code: code! }).returning();
...
export async function sendReferralInvitation(
  ...
  if (!result.success) {
    logger.error('Failed to send referral invitation', {
      inviterEmail,
      inviteeEmail,
      error: result.error,
    });
...
export async function redeemReferralCode(
  ...
  const [inviter] = await db
    ...
    .limit(1);
  const template = referralCouponReceived(
    newUserName,
    couponCode,
    REFERRAL_INVITEE_CENTS / 100,
    'welcome',
  );
...
export async function triggerInviterReward(buyerId: string): Promise<void> {
  ...
  const [refCode] = await db
    ...
    .from(referralCodes)
    .where(eq(referralCodes.id, invitation.referralCodeId))
    .limit(1);
  if (!refCode) return;
  ...
  logger.info('Inviter reward issued', {
...
export async function getReferralStats(userId: string) {
  ...
  const [{ total }] = await db
    .select({ total: count() })
    .from(referralInvitations)
    .where(eq(referralInvitations.referralCodeId, refCode.id));
