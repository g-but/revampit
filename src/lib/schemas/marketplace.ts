...
export const CreateListingSchema = z.object({
  title: z
    ...
    .max(
      MARKETPLACE_LIMITS.MAX_TITLE_LENGTH,
      `Titel darf maximal ${MARKETPLACE_LIMITS.MAX_TITLE_LENGTH} Zeichen lang sein`,
    ),
  description: z
    .string()
    .min(10, 'Beschreibung muss mindestens 10 Zeichen lang sein')
    .max(
      MARKETPLACE_LIMITS.MAX_DESCRIPTION_LENGTH,
      `Beschreibung darf maximal ${MARKETPLACE_LIMITS.MAX_DESCRIPTION_LENGTH} Zeichen lang sein`,
    ),
  price_chf: z
    .number()
    .min(0, 'Preis muss mindestens 0 sein')
    .max(
      MARKETPLACE_LIMITS.MAX_PRICE_CHF,
      `Preis darf maximal ${MARKETPLACE_LIMITS.MAX_PRICE_CHF} CHF sein`,
    ),
  category: z.enum(MARKETPLACE_CATEGORY_VALUES, {
    error: 'Ungültige Kategorie',
  }),
  condition: z
    .enum(LISTING_CONDITIONS as unknown as [string, ...string[]], {
...
export const UpdateListingSchema = z.object({
  ...
  images: z
    ...
    .max(MARKETPLACE_LIMITS.MAX_IMAGES)
    .optional(),
...
export const AskListingQuestionSchema = z.object({
  question: z
    ...
    .max(
      MARKETPLACE_LIMITS.MAX_QUESTION_LENGTH,
      `Frage darf maximal ${MARKETPLACE_LIMITS.MAX_QUESTION_LENGTH} Zeichen lang sein`,
    ),
});
...
export const AnswerListingQuestionSchema = z.object({
  answer: z
    ...
    .max(
      MARKETPLACE_LIMITS.MAX_ANSWER_LENGTH,
      `Antwort darf maximal ${MARKETPLACE_LIMITS.MAX_ANSWER_LENGTH} Zeichen lang sein`,
    ),
});
...
export const AdminListingsQuerySchema = z
  .object({
    status: z.enum(['all', ...LISTING_STATUSES] as [string, ...string[]]).default('all'),
    category: z.string().optional(),
    seller_type: z
      .enum(['all', ...Object.values(MARKETPLACE_SELLER_TYPE)] as [string, ...string[]])
      .default('all'),
