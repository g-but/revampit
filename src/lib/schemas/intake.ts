...
import { CAPTURE_DESTINATIONS, CAPTURE_DESTINATION_VALUES } from '@/config/intake-workflow';
...
const intakeTierSchema = z.enum([INTAKE_TIERS.REFURBISH, INTAKE_TIERS.PARTS, INTAKE_TIERS.RECYCLE]);

export const IntakeCreateSchema = erfassungPayloadSchema
  .omit({ action: true, publish: true })
  .extend({
  ...
  .refine(
    (data) =>
      data.destination !== CAPTURE_DESTINATIONS.SHOP_UNTESTED ||
  ...
  .refine(
    ...
    {
      message:
        'Für eine direkte Veröffentlichung ist ein Verkaufspreis grösser als null erforderlich',
      path: ['verkaufspreis'],
    },
  );
...
export const ChecklistUpdateSchema = z
  ...
  .refine((data) => Boolean(data.item_id) !== Boolean(data.item_ids), {
    message: 'Entweder item_id oder item_ids angeben',
    path: ['item_id'],
  })
  .refine(
    (data) =>
      !data.item_ids || (data.result === CHECKLIST_RESULTS.PASS && !data.second_person_override),
    { message: 'Sammel-Update erlaubt nur "pass" ohne Override', path: ['item_ids'] },
  )
  .refine((data) => data.result !== CHECKLIST_RESULTS.FAIL || data.notes.trim().length > 0, {
    message: ERROR_MESSAGES.INTAKE_FAIL_NOTES_REQUIRED,
    path: ['notes'],
  })
  .refine((data) => !data.second_person_override || data.notes.trim().length >= 10, {
    message: ERROR_MESSAGES.INTAKE_SECOND_PERSON_OVERRIDE_REASON_REQUIRED,
    path: ['notes'],
  });
...
export const IntakePublishSchema = z
  ...
  .refine((data) => !data.qc_skip || data.qc_skip_reason.trim().length >= 10, {
    message: ERROR_MESSAGES.INTAKE_QC_SKIP_REASON_REQUIRED,
    path: ['qc_skip_reason'],
  });
