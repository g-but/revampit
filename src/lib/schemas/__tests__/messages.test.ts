...
describe('SendMessageSchema', () => {
  ...
  it('caps content at 5000 characters', () => {
    ...
    expect(SendMessageSchema.safeParse({ ...valid, content: 'x'.repeat(5001) }).success).toBe(
      false,
    );
  });
...
describe('CreateConversationSchema', () => {
  ...
  it('accepts optional contextId as null or a UUID', () => {
    ...
    expect(CreateConversationSchema.safeParse({ ...valid, contextId: OTHER_UUID }).success).toBe(
      true,
    );
  });
  ...
  it('caps initialMessage at 5000 characters when provided', () => {
    expect(
      CreateConversationSchema.safeParse({ ...valid, initialMessage: 'x'.repeat(5000) }).success,
    ).toBe(true);
    expect(
      CreateConversationSchema.safeParse({ ...valid, initialMessage: 'x'.repeat(5001) }).success,
    ).toBe(false);
  });

  it('rejects empty initialMessage when explicitly provided', () => {
    expect(CreateConversationSchema.safeParse({ ...valid, initialMessage: '' }).success).toBe(
      false,
    );
  });

  it('caps type at 50 characters', () => {
    expect(CreateConversationSchema.safeParse({ ...valid, type: 'x'.repeat(51) }).success).toBe(
      false,
    );
  });
});
