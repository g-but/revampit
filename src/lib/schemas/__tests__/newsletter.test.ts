...
describe('NewsletterSubscribeSchema', () => {
  ...
  it('rejects whitespace-padded email (Zod email() runs before transform)', () => {
    ...
    expect(NewsletterSubscribeSchema.safeParse({ email: '  anna@example.com  ' }).success).toBe(
      false,
    );
  });
  ...
  it('rejects a name longer than 100 chars', () => {
    expect(
      NewsletterSubscribeSchema.safeParse({ email: 'a@b.com', name: 'x'.repeat(101) }).success,
    ).toBe(false);
  });
  ...
  it('rejects a source longer than 50 chars', () => {
    expect(
      NewsletterSubscribeSchema.safeParse({ email: 'a@b.com', source: 'x'.repeat(51) }).success,
    ).toBe(false);
  });
});
