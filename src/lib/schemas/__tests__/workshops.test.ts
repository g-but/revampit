...
describe('WorkshopRegistrationSchema', () => {
  it('accepts a valid workshop slug', () => {
    expect(WorkshopRegistrationSchema.safeParse({ workshopSlug: 'linux-basics' }).success).toBe(
      true,
    );
  });
...
describe('WorkshopProposalSchema', () => {
  ...
  it('caps title at 200 chars', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, title: 'x'.repeat(201) }).success,
    ).toBe(false);
  });

  it('caps shortDescription at 500 chars', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, shortDescription: 'x'.repeat(501) })
        .success,
    ).toBe(false);
  });
  ...
  it('rejects empty learningObjectives array', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, learningObjectives: [] }).success,
    ).toBe(false);
  });

  it('rejects when termsAccepted is false', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, termsAccepted: false }).success,
    ).toBe(false);
  });
  ...
  it('rejects non-positive durationHours', () => {
    expect(WorkshopProposalSchema.safeParse({ ...validProposal, durationHours: 0 }).success).toBe(
      false,
    );
    expect(WorkshopProposalSchema.safeParse({ ...validProposal, durationHours: -1 }).success).toBe(
      false,
    );
  });
  ...
  it('coerces participant counts from strings and requires integers ≥ 1', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, maxParticipants: '12' }).success,
    ).toBe(true);
    expect(WorkshopProposalSchema.safeParse({ ...validProposal, maxParticipants: 0 }).success).toBe(
      false,
    );
    expect(WorkshopProposalSchema.safeParse({ ...validProposal, minParticipants: 0 }).success).toBe(
      false,
    );
  });

  it('accepts pricePerPerson of 0 (free workshop)', () => {
    expect(WorkshopProposalSchema.safeParse({ ...validProposal, pricePerPerson: 0 }).success).toBe(
      true,
    );
  });
  ...
  it('rejects negative pricePerPerson', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, pricePerPerson: -10 }).success,
    ).toBe(false);
  });

  it('accepts optional selectedLocationId as null or a UUID', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, selectedLocationId: null }).success,
    ).toBe(true);
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, selectedLocationId: UUID }).success,
    ).toBe(true);
  });

  it('rejects a non-UUID selectedLocationId', () => {
    expect(
      WorkshopProposalSchema.safeParse({ ...validProposal, selectedLocationId: 'abc' }).success,
    ).toBe(false);
  });
