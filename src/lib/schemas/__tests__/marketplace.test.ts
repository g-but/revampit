...
describe('CreateListingSchema', () => {
  ...
  it('rejects title longer than MAX_TITLE_LENGTH', () => {
    const result = CreateListingSchema.safeParse({
      ...valid,
      title: 'x'.repeat(MARKETPLACE_LIMITS.MAX_TITLE_LENGTH + 1),
    });
    expect(result.success).toBe(false);
  });
  ...
  it('accepts title of exactly MAX_TITLE_LENGTH', () => {
    const result = CreateListingSchema.safeParse({
      ...valid,
      title: 'x'.repeat(MARKETPLACE_LIMITS.MAX_TITLE_LENGTH),
    });
    expect(result.success).toBe(true);
  });
  ...
  it('rejects price above MAX_PRICE_CHF', () => {
    const result = CreateListingSchema.safeParse({
      ...valid,
      price_chf: MARKETPLACE_LIMITS.MAX_PRICE_CHF + 1,
    });
    expect(result.success).toBe(false);
  });
  ...
  it('accepts price of exactly MAX_PRICE_CHF', () => {
    const result = CreateListingSchema.safeParse({
      ...valid,
      price_chf: MARKETPLACE_LIMITS.MAX_PRICE_CHF,
    });
    expect(result.success).toBe(true);
  });
  ...
  it('rejects more than MAX_IMAGES images', () => {
    const images = Array.from(
      { length: MARKETPLACE_LIMITS.MAX_IMAGES + 1 },
      (_, i) => `/uploads/img${i}.jpg`,
    );
...
describe('AskListingQuestionSchema', () => {
  ...
  it('rejects question longer than MAX_QUESTION_LENGTH', () => {
    const result = AskListingQuestionSchema.safeParse({
      question: 'x'.repeat(MARKETPLACE_LIMITS.MAX_QUESTION_LENGTH + 1),
    });
    expect(result.success).toBe(false);
  });
});
...
describe('AnswerListingQuestionSchema', () => {
  it('accepts valid answer', () => {
    const result = AnswerListingQuestionSchema.safeParse({
      answer: 'Ja, Akku hält noch ca. 4 Stunden.',
    });
    expect(result.success).toBe(true);
  });
...
describe('ListingsQuerySchema', () => {
  ...
  it('rejects price_max above MAX_PRICE_CHF', () => {
    const result = ListingsQuerySchema.safeParse({
      price_max: MARKETPLACE_LIMITS.MAX_PRICE_CHF + 1,
    });
    expect(result.success).toBe(false);
  });
