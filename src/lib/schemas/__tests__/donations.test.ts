...
describe('CreateMonetaryDonationSchema', () => {
  ...
  it('rejects non-integer amount_cents', () => {
    const result = CreateMonetaryDonationSchema.safeParse({ ...valid, amount_cents: 10.5 });
    expect(result.success).toBe(false);
  });
...
describe('DonationDropoffSchema', () => {
  ...
  it('rejects an invalid email', () => {
    expect(DonationDropoffSchema.safeParse({ ...MIN_VALID, email: 'not-an-email' }).success).toBe(
      false,
    );
  });
  ...
  it('rejects notes over 2000 chars', () => {
    expect(DonationDropoffSchema.safeParse({ ...MIN_VALID, notes: 'x'.repeat(2001) }).success).toBe(
      false,
    );
  });
});
