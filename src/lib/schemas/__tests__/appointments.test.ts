...
describe('CreateAppointmentSchema', () => {
  ...
  it('caps description at 2000 chars', () => {
    expect(CreateAppointmentSchema.safeParse({ description: 'x'.repeat(2001) }).success).toBe(
      false,
    );
  });
  ...
  it('rejects an unknown urgency', () => {
    expect(CreateAppointmentSchema.safeParse({ ...valid, urgency: 'whenever' }).success).toBe(
      false,
    );
  });
  ...
  it('rejects a non-ISO preferred_date', () => {
    expect(
      CreateAppointmentSchema.safeParse({ ...valid, preferred_date: '2026-06-15' }).success,
    ).toBe(false);
  });

  it('accepts ISO datetime with offset for both naming variants', () => {
    expect(CreateAppointmentSchema.safeParse({ ...valid, preferred_date: ISO_DATE }).success).toBe(
      true,
    );
    expect(CreateAppointmentSchema.safeParse({ ...valid, preferredDate: ISO_DATE }).success).toBe(
      true,
    );
  });
...
describe('BookWithPaymentSchema', () => {
  ...
  it('clamps autoReleaseDays to [1, 90]', () => {
    expect(BookWithPaymentSchema.safeParse({ serviceSlug: 'x', autoReleaseDays: 0 }).success).toBe(
      false,
    );
    expect(BookWithPaymentSchema.safeParse({ serviceSlug: 'x', autoReleaseDays: 91 }).success).toBe(
      false,
    );
    expect(BookWithPaymentSchema.safeParse({ serviceSlug: 'x', autoReleaseDays: 1 }).success).toBe(
      true,
    );
  });
});
...
describe('AppointmentActionSchema', () => {
  ...
  it('requires positive quoted_price_chf for quote action', () => {
    ...
    expect(
      AppointmentActionSchema.safeParse({ action: 'quote', quoted_price_chf: 0 }).success,
    ).toBe(false);
    expect(
      AppointmentActionSchema.safeParse({ action: 'quote', quoted_price_chf: 150 }).success,
    ).toBe(true);
  });
  ...
  it('start action accepts optional confirmed_date as ISO', () => {
    ...
    expect(
      AppointmentActionSchema.safeParse({ action: 'start', confirmed_date: ISO_DATE }).success,
    ).toBe(true);
    expect(
      AppointmentActionSchema.safeParse({ action: 'start', confirmed_date: '2026-06-15' }).success,
    ).toBe(false);
  });

  it('rate action requires customer_rating in [1,5]', () => {
    expect(AppointmentActionSchema.safeParse({ action: 'rate' }).success).toBe(false);
    expect(AppointmentActionSchema.safeParse({ action: 'rate', customer_rating: 0 }).success).toBe(
      false,
    );
    expect(AppointmentActionSchema.safeParse({ action: 'rate', customer_rating: 6 }).success).toBe(
      false,
    );
    expect(AppointmentActionSchema.safeParse({ action: 'rate', customer_rating: 3 }).success).toBe(
      true,
    );
  });
