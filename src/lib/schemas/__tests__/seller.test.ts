...
describe('SellerApplicationSchema', () => {
  ...
  it('requires Swiss postal code format (4 digits)', () => {
    ...
    expect(SellerApplicationSchema.safeParse({ ...valid, postalCode: '80050' }).success).toBe(
      false,
    );
  ...
  it('rejects when termsAccepted is false', () => {
    expect(SellerApplicationSchema.safeParse({ ...valid, termsAccepted: false }).success).toBe(
      false,
    );
  });
