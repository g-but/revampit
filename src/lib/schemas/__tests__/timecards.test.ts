...
describe('reviewTimecardSchema', () => {
  it('accepts approve and reject statuses only', () => {
    expect(reviewTimecardSchema.safeParse({ status: TIMECARD_STATUSES.APPROVED }).success).toBe(
      true,
    );
    expect(reviewTimecardSchema.safeParse({ status: TIMECARD_STATUSES.REJECTED }).success).toBe(
      true,
    );
    expect(reviewTimecardSchema.safeParse({ status: TIMECARD_STATUSES.SUBMITTED }).success).toBe(
      false,
    );
  });
});
