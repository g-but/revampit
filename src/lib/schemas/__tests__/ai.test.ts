...
describe('AnalyzeProductSchema', () => {
  it('accepts an inline image alone', () => {
    expect(AnalyzeProductSchema.safeParse({ image: 'data:image/jpeg;base64,...' }).success).toBe(
      true,
    );
  });
  ...
  it('accepts an imageUrl alone', () => {
    expect(
      AnalyzeProductSchema.safeParse({ imageUrl: 'https://example.com/img.jpg' }).success,
    ).toBe(true);
  });

  it('accepts both together', () => {
    expect(
      AnalyzeProductSchema.safeParse({ image: 'data:...', imageUrl: 'https://example.com/img.jpg' })
        .success,
    ).toBe(true);
  });
