...
describe('CreatePaymentIntentSchema', () => {
  ...
  it('clamps autoReleaseDays to [1, 90]', () => {
    expect(CreatePaymentIntentSchema.safeParse({ amount: 100, autoReleaseDays: 0 }).success).toBe(
      false,
    );
    expect(CreatePaymentIntentSchema.safeParse({ amount: 100, autoReleaseDays: 91 }).success).toBe(
      false,
    );
    expect(CreatePaymentIntentSchema.safeParse({ amount: 100, autoReleaseDays: 1 }).success).toBe(
      true,
    );
    expect(CreatePaymentIntentSchema.safeParse({ amount: 100, autoReleaseDays: 90 }).success).toBe(
      true,
    );
  });
...
describe('RefundActionSchema', () => {
  ...
  it('caps notes at 2000 characters', () => {
    expect(
      RefundActionSchema.safeParse({ action: 'approve', notes: 'x'.repeat(2001) }).success,
    ).toBe(false);
  });
});
