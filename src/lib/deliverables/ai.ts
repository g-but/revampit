...
function sourceTypeFor(name: string): 'markdown' | 'code' | 'text' | 'json' {
  ...
  if (['html', 'htm', 'css', 'js', 'ts', 'tsx', 'jsx', 'svg', 'xml', 'yml', 'yaml'].includes(ext))
    return 'code';
  return 'text';
}
...
export async function ingestDeliverable(deliverable: DeliverableDetail): Promise<void> {
  try {
    ...
    logger.info('Deliverable ingested into Hirn RAG', {
...
export async function answerDeliverableQuestion(
  ...
  try {
    ...
    const other = results.filter(
      (r) => !r.document.sourcePath.startsWith(`${RAG_PREFIX}/${deliverable.id}/`),
    );
    if (other.length)
