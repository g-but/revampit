...
describe('locationApprovalNotification', () => {
  ...
  it('html contains the submitter name', () => {
    expect(locationApprovalNotification('Anna', 'Bern Altstadt', 'approve', null).html).toContain(
      'Anna',
    );
  });
  ...
  it('with review notes → html contains them', () => {
    const email = locationApprovalNotification(
      'Anna',
      'Bern',
      'reject',
      'Bitte Adresse vervollständigen',
    );
...
describe('locationSubmissionConfirmation', () => {
  ...
  it('html contains the location name', () => {
    expect(locationSubmissionConfirmation('Max', 'Hauptbahnhof', 'Zürich').html).toContain(
      'Hauptbahnhof',
    );
  });
