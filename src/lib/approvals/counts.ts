 ...
 */
const COUNT_DESCRIPTORS: Record<
  string,
  { table: string; statusColumn: string; pendingValue: string; timeColumn: string }
> = {
  blog: {
    table: TABLE_NAMES.BLOG_SUBMISSIONS,
    statusColumn: 'status',
    pendingValue: APPROVAL_STATUS.PENDING,
    timeColumn: 'created_at',
  },
  workshop_proposal: {
    table: TABLE_NAMES.WORKSHOP_PROPOSALS,
    statusColumn: 'status',
    pendingValue: APPROVAL_STATUS.PENDING,
    timeColumn: 'created_at',
  },
  ...
  timecard: {
    table: 'timecards',
    statusColumn: 'status',
    pendingValue: TIMECARD_STATUSES.SUBMITTED,
    timeColumn: 'submitted_at',
  },
  time_off: {
    table: TABLE_NAMES.TIME_OFF_REQUESTS,
    statusColumn: 'status',
    pendingValue: TIME_OFF_STATUSES.PENDING,
    timeColumn: 'created_at',
  },
  permission_request: {
    table: TABLE_NAMES.STAFF_PERMISSION_REQUESTS,
    statusColumn: 'status',
    pendingValue: PERMISSION_REQUEST_STATUS.PENDING,
    timeColumn: 'created_at',
  },
};
...
export function oldestPendingAt(counts: ApprovalCounts): string | null {
  const times = Object.values(counts)
    .map((c) => c.oldestAt)
