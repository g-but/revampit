 ...
 */
async function extractChunk(text: string, sourceType: 'text' | 'voice'): Promise<BulkProduct[]> {
  try {
  ...
  return lines.map((line) => {
    ...
    if (priceMatch) {
      ...
      if (price >= 10 && price <= 9999) {
        ...
        remaining = remaining
          .replace(new RegExp(priceMatch[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s*$'), '')
          .trim();
      }
    }
    ...
    const sortedAliases = Object.entries(CONDITION_ALIASES).sort(
 ...
 */
function voiceDataToBulkProduct(data: VoiceProductData, sourceType: 'text' | 'voice'): BulkProduct {
  const formData: Partial<ErfassungFormData> = {
