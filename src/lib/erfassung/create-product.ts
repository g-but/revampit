...
export async function createErfassungProduct(
  ...
  if (payload.langtext) {
    try {
      specifications =
        typeof payload.langtext === 'string' ? JSON.parse(payload.langtext) : payload.langtext;
    } catch {
  ...
  const productStatus =
    action === 'draft' ? PRODUCT_STATUS.PENDING_REVIEW : PRODUCT_STATUS.APPROVED;
  const marketplaceStatus =
    action === 'publish' ? MARKETPLACE_STATUS.PUBLISHED : MARKETPLACE_STATUS.DRAFT;

  const qcBypassed = action === 'publish' && Boolean(options?.qcBypassReason?.trim());
  ...
  const finalMarketplaceStatus =
    options?.checklistGated || qcRequired ? MARKETPLACE_STATUS.DRAFT : marketplaceStatus;
