...
const TECH_REVIEW_SITES = [
  {
    ...
    buildUrl: (query: string) =>
      `https://www.notebookcheck.com/Search.8222.0.html?q=${encodeURIComponent(query)}`,
 ...
 */
export function generateVerificationSources(data: VoiceProductData): {
  fieldSources: Record<string, VerificationSource[]>;
  allSources: VerificationSource[];
} {
  ...
  fieldSources.specs = [
    ...(fieldSources.specs || []),
    ...reviewSources.filter((s) => s.type === 'review'),
  ];
...
export function calculateFieldConfidence(
  ...
  if (data.specs?.length) {
    const specsConfidence = ['ram', 'gb', 'ssd', 'cpu', 'i5', 'i7', 'ghz', 'core'].some((kw) =>
