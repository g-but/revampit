...
export function detectCategory(text: string): string {
  ...
  const patterns: Array<[RegExp, string]> = [
    [
      /\b(drucker|laserjet|pixma|officejet|deskjet|i-?sensys|imageclass|maxify|multifunktion|printer|scanner|toner)\b|\b(mfc|dcp)[-\s]/i,
      code('Drucker & Scanner'),
    ],
    [
      /\b(ultrasharp|ultrawide|monitor|bildschirm|beamer|projektor|eizo|zoll[\s-]*(led|lcd|tft))\b/i,
      code('Monitore'),
    ],
    [
      /\b(dockingstation|dock|router|switch|access\s*point|firewall|netzwerk(adapter)?|wlan|nas|tp-?link|archer|\bmodem\b|converged\s*network|(ethernet|network|server)\s*adapter|ubee)\b/i,
      code('Netzwerk'),
    ],
    ...
    [
      /\b(thinkpad|latitude|elitebook|probook|macbook|ideapad|inspiron|pavilion|vivobook|zenbook|chromebook|swift|aspire|travelmate|lifebook|\bxps\b|laptop|notebook|ultrabook)\b/i,
      code('Laptops'),
    ],
    [
      /\b(optiplex|prodesk|thinkcentre|imac|mac\s*mini|nuc|elitedesk|esprimo|compaq\s*\d|desktop|tower|workstation|power\s*macintosh|pc)\b/i,
      code('Desktop PCs'),
    ],
...
export function fastParseProductText(text: string): VoiceProductData {
  ...
  const modelMatch = text.match(
    /(?:latitude|thinkpad|elitebook|probook|macbook|surface|inspiron|xps|pavilion)\s*[\w\-]+/i,
  );
  ...
  const cpuMatch = text.match(
    /i[357]\s*[-]?\s*\d{4,5}[a-z]?|ryzen\s*\d|m[123]\s*(pro|max)?|core\s*\d/i,
  );
  ...
  const verkaufspreis = priceWithCurrency
    ? priceWithCurrency[1]
    : priceAtEnd && parseInt(priceAtEnd[1]) >= 50 && parseInt(priceAtEnd[1]) <= 9999
      ? priceAtEnd[1]
      : '';
  ...
  const unterkategorie =
    hauptkategorie === catLaptops.value
      ? textLower.includes('gaming')
        ? subGaming
        : textLower.includes('business') ||
            textLower.includes('latitude') ||
            textLower.includes('thinkpad')
          ? subBusiness
          : subConsumer
      : '';
