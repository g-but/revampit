...
const buildExtractionPrompt = (text: string) =>
  fillPromptTemplate(ERFASSUNG_PROMPTS.extract, {
    text,
  ...
  if (result) {
    ...
    if (jsonStr) {
      try {
        ...
        const { metadata, allSources } = calculateFieldConfidence(
          text,
          productData,
          sourceType,
          result.model,
 ...
 */
export async function extractProductFromImage(imageBase64: string): Promise<ExtractProductResult> {
