...
import { generateVerificationSources, calculateFieldConfidence } from '../ai-field-mapping';
...
describe('calculateFieldConfidence — verkaufspreis price pattern', () => {
  ...
  it('confidence 0.9 when input contains a price-like pattern', () => {
    expect(
      calculateFieldConfidence('500 chf', dataWithPrice, 'text').metadata.verkaufspreis?.confidence,
    ).toBe(0.9);
    expect(
      calculateFieldConfidence('300 franken', dataWithPrice, 'text').metadata.verkaufspreis
        ?.confidence,
    ).toBe(0.9);
    expect(
      calculateFieldConfidence('250 fr', dataWithPrice, 'text').metadata.verkaufspreis?.confidence,
    ).toBe(0.9);
    expect(
      calculateFieldConfidence('100.-', dataWithPrice, 'text').metadata.verkaufspreis?.confidence,
    ).toBe(0.9);
  });
