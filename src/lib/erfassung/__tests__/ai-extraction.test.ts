...
const mockFillPromptTemplate = vi.fn((template: string) => `PROMPT:${template}`);
...
vi.mock('@/lib/ai/config/prompts', () => ({
  ...
  fillPromptTemplate: (...args: unknown[]) => mockFillPromptTemplate.apply(null, args as never),
}));
...
describe('extractProductFromImage', () => {
  ...
  it('normalises a bare base64 string into a full data URL', async () => {
    ...
    expect(mockCallVisionWithFallback.mock.calls[0][0].imageDataUrl).toBe(
  ...
  it('returns an error when no JSON is found in the vision response', async () => {
    mockCallVisionWithFallback.mockResolvedValueOnce(
      visionOk('Ich sehe ein Laptop aber kein JSON'),
    );
