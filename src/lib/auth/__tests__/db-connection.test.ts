...
vi.mock('pg', () => ({
  // function, not arrow: vitest invokes the implementation with `new`,
...
vi.mock('../config', () => ({
  getDbConfig: vi.fn(() => ({ host: 'localhost', database: 'test' })),
}));
...
beforeEach(async () => {
  ...
  dbConn = (await import('../db-connection')) as typeof import('../db-connection');
});
...
describe('paginatedQuery', () => {
  ...
  it('passes pagination parameters through', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 });
    await dbConn.paginatedQuery('SELECT id FROM x WHERE status = $1 LIMIT $2 OFFSET $3', [
      'active',
      20,
      40,
    ]);
