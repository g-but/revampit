...
export async function getWorkshopsForUser(
  userId: string,
): Promise<Array<DbWorkshop & { registration_status?: string }>> {
  const rows = await db
    .select({
      id: workshops.id,
      slug: workshops.slug,
 ...
 */
export async function getUserWorkshopRegistrations(
  userId: string,
): Promise<Array<DbWorkshopRegistration & { workshop_title: string; workshop_slug: string }>> {
  const rows = await db
    .select({
