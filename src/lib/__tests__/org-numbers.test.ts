...
vi.mock('@/db/schema', () => ({
  orgNumbers: {
    key: 'on_key',
    value: 'on_value',
    numericValue: 'on_numericValue',
    label: 'on_label',
    category: 'on_category',
    confidence: 'on_confidence',
    methodology: 'on_methodology',
    calculation: 'on_calculation',
    sourceDocument: 'on_sourceDocument',
    externalLink: 'on_externalLink',
    lastVerified: 'on_lastVerified',
    updatedAt: 'on_updatedAt',
  },
}));
...
describe('getOrgNumbers', () => {
  ...
  it('returns all mapped rows when no category filter', async () => {
    mockDbSelect.mockReturnValueOnce(
      makeChain([
        makeRow({ key: 'founding_year' }),
        makeRow({ key: 'team_fte', value: '5', numericValue: 5 }),
      ]),
    );
  ...
  it('passes category filter to query when provided', async () => {
    ...
    const categoryCall = eq.mock.calls.find(([, v]) => v === 'impact');
