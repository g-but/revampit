...
describe('canAccessSensitive', () => {
  ...
  it('returns false for non-staff', () => {
    expect(
      canAccessSensitive({ email: 'x@gmail.com', is_staff: false, staff_permissions: [] }),
    ).toBe(false);
  });
  ...
  it('wildcard permission grants sensitive access', () => {
    expect(
      canAccessSensitive({ email: 'x@revamp-it.ch', is_staff: true, staff_permissions: ['*'] }),
    ).toBe(true);
  });

  it('staff with a sensitive section permission can access sensitive', () => {
    expect(
      canAccessSensitive({ email: 'x@revamp-it.ch', is_staff: true, staff_permissions: ['users'] }),
    ).toBe(true);
  });
...
describe('getAccessibleSections', () => {
  ...
  it('returns empty array for non-staff', () => {
    expect(
      getAccessibleSections({ email: 'x@gmail.com', is_staff: false, staff_permissions: [] }),
    ).toEqual([]);
  });
