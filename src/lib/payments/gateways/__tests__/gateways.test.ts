...
describe('btcpay HMAC verification (configured)', () => {
  ...
  it('accepts a correctly signed delivery', async () => {
    ...
    const ok = await btcpayGateway.verifyWebhook(
      body,
      new Headers({ 'btcpay-sig': `sha256=${sig}` }),
    );
    expect(ok).toBe(true);
  });
  ...
  it('rejects a wrong signature', async () => {
    const ok = await btcpayGateway.verifyWebhook(
      body,
      new Headers({ 'btcpay-sig': 'sha256=deadbeef' }),
    );
