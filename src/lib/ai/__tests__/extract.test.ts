...
import { robustJsonExtract, calculateGenericConfidence } from '../extract';
...
describe('robustJsonExtract — plain JSON', () => {
  ...
  it('parses nested objects', () => {
    expect(robustJsonExtract('{"user":{"name":"Anna","age":30}}')).toEqual({
      user: { name: 'Anna', age: 30 },
    });
  });

  it('parses arrays inside objects', () => {
    expect(robustJsonExtract('{"tags":["a","b","c"]}')).toEqual({ tags: ['a', 'b', 'c'] });
  });
  ...
  it('parses booleans and numbers correctly', () => {
    expect(robustJsonExtract('{"enabled":true,"count":42,"price":19.95}')).toEqual({
      enabled: true,
      count: 42,
      price: 19.95,
    });
