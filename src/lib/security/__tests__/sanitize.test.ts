...
describe('sanitizeObject', () => {
  ...
  it('sanitizes multiple listed fields', () => {
    const result = sanitizeObject({ a: '<b>1</b>', b: '<i>2</i>', c: 3 }, ['a', 'b']);
    expect(result.a).toBe('1');
    expect(result.b).toBe('2');
  });

  it('leaves non-string fields untouched even when listed', () => {
    const result = sanitizeObject({ count: 42, flag: true, name: '<b>x</b>' }, [
      'count',
      'flag',
      'name',
    ]);
  ...
  it('passes options through to sanitizeInput (allowHtml + maxLength)', () => {
    ...
    const result = sanitizeObject({ content: longBold }, ['content'], {
      allowHtml: true,
      maxLength: 20,
    });
