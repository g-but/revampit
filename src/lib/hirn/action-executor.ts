...
import { type ExecuteActionInput, isRiskyAction } from './action-executor-contracts';
  ...
  switch (input.actionType) {
    ...
    case 'create_decision_draft': {
      ...
      const decision = await createDecision(
        { ...parsed.data, initialStatus: DECISION_STATUS.DRAFT },
        dbUserId,
      );
      return {
        mode: 'executed' as const,
        entity: {
          ...
          link: `/admin/decisions/${decision.id}`,
        },
    ...
    case 'create_protocol_draft': {
      ...
      return {
        ...
        entity: {
          ...
          id: protocol.id,
          title: parsed.data.title,
          link: `/admin/protocols/${protocol.id}`,
        },
...
function buildPreview(
  actionType: ExecuteActionInput['actionType'],
