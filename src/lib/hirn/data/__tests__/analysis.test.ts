...
import type { YearlyAggregation, TracedValue, MonthlyData } from '../financial-loader';
...
describe('analyzeSeasonality', () => {
  it('flags strong + weak months when seasonality is clear', () => {
    ...
    const monthlyTotals = [
      12_000, 12_000, 12_000, 12_000, 7_500, 7_500, 7_500, 7_500, 3_000, 3_000, 3_000, 3_000,
    ];
  ...
  it('strongMonths contains month numbers (1..12), not array indices', () => {
    const monthlyTotals = [
      20_000, 1_000, 1_000, 1_000, 1_000, 1_000, 1_000, 1_000, 1_000, 1_000, 1_000, 1_000,
    ];
    const result = analyzeSeasonality(buildYear({ monthlyTotals }));
...
describe('generateYearInsights', () => {
  ...
  it('emits "zero_months" warning when at least one monthly total ≤ 0', () => {
    const monthlyTotals = [
      10_000, 10_000, 0, 10_000, 10_000, 10_000, 10_000, 10_000, 10_000, 10_000, 10_000, 10_000,
    ];
  ...
  it('emits "service_heavy" (positive) when dienstleistungen > 50% of total', () => {
    const insights = generateYearInsights(
      buildYear({ total: 100_000, dienstleistungen: 55_000, warenverkauf: 30_000 }),
    );
