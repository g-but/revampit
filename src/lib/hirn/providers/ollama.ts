...
export class OllamaProvider implements AIProvider {
  ...
  constructor(config: ProviderConfig = {}) {
    ...
    this.embeddingModel =
      config.embeddingModel || process.env.OLLAMA_EMBEDDING_MODEL || DEFAULT_EMBEDDING_MODEL;
  }
    ...
    for (const input of inputs) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Ollama embeddings error: ${response.status} - ${error.substring(0, 200)}`,
          );
        }
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          throw new Error(
            `Ollama embedding Zeitüberschreitung nach ${EMBEDDING_TIMEOUT_MS / 1000}s`,
          );
        }
        throw error;
      } finally {
