...
export async function searchSimilar(
  ...
  const { topK = 5, minSimilarity = 0.5, sourceTypes, sourcePaths } = options;
  ...
  if (sourceTypes && sourceTypes.length > 0) {
    filterParts.push(
      sql`AND d.source_type IN (${sql.join(
  ...
  const filterClause = filterParts.length > 0 ? sql.join(filterParts, sql` `) : sql``;
...
export async function listDocuments(options: {
  ...
  const whereClause = sourceType ? sql`WHERE d.source_type = ${sourceType}` : sql``;
  ...
  const countCondition = sourceType ? eq(hirnDocuments.sourceType, sourceType) : undefined;

  const [countRow] = await db.select({ count: count() }).from(hirnDocuments).where(countCondition);
