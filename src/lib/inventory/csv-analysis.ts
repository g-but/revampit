...
export function analyzeProductDescription(
  ...
  if (
    ...
    desc.includes('ssd') ||
    desc.includes('hdd') ||
    desc.includes('festplatte') ||
    desc.includes('hard drive')
  ) {
    category = 'Computer-Komponenten';
    confidence = 0.8;
  } else if (
    desc.includes('tastatur') ||
  ...
  if (desc.includes('neu') || desc.includes('new') || desc.includes('unbenutzt')) {
    ...
    desc.includes('wie neu') ||
    desc.includes('excellent') ||
    desc.includes('ausgezeichnet')
  ) {
    condition = 'excellent';
...
export function calculateSustainabilityScore(analysis: ProductAnalysis) {
  ...
  const hasSustainableBrand = sustainableBrands.some((brand) =>
    analysis.brand?.toLowerCase().includes(brand),
  );
