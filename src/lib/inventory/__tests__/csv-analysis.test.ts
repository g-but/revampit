...
import { analyzeProductDescription, calculateSustainabilityScore } from '../csv-analysis';
...
describe('analyzeProductDescription — categories', () => {
  it('detects Laptops from "laptop" keyword', () => {
    expect(analyzeProductDescription('Used laptop, good condition', 'Lenovo').category).toBe(
      'Laptops',
    );
  });
...
describe('calculateSustainabilityScore', () => {
  ...
  it('matches sustainable brands case-insensitively + as substrings', () => {
    expect(
      calculateSustainabilityScore({ ...baseAnalysis, brand: 'APPLE INC.' }).overall_score,
  ...
  it('adds 5 for "excellent" condition', () => {
    expect(
      calculateSustainabilityScore({ ...baseAnalysis, condition: 'excellent' }).overall_score,
    ).toBe(55);
  });
  ...
  it('refurbishment_benefit is high (80) when condition is not new', () => {
    expect(
      calculateSustainabilityScore({ ...baseAnalysis, condition: 'good' }).factors
        .refurbishment_benefit,
    ).toBe(80);
  });

  it('refurbishment_benefit drops to 30 for new condition (penalty)', () => {
    expect(
      calculateSustainabilityScore({ ...baseAnalysis, condition: 'new' }).factors
        .refurbishment_benefit,
    ).toBe(30);
  });
  ...
  it('product_recyclability is 70 for Laptops, 50 otherwise', () => {
    expect(
      calculateSustainabilityScore({ ...baseAnalysis, category: 'Laptops' }).factors
        .product_recyclability,
    ).toBe(70);
    expect(
      calculateSustainabilityScore({ ...baseAnalysis, category: 'Smartphones' }).factors
        .product_recyclability,
    ).toBe(50);
  });
