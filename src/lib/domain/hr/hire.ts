import { ROLE_TRACK_TO_EMPLOYMENT_TYPE, type RoleTrack } from '@/config/hr-vacancies';
...
export function extractProfileFromTrackResponses(
  ...
  const skills = Array.isArray(trackResponses.skills) ? (trackResponses.skills as string[]) : [];
  ...
  const situation = typeof trackResponses.situation === 'string' ? trackResponses.situation : null;
