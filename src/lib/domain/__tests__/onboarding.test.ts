...
describe('isTeamProfileComplete (staff)', () => {
  it('needs both skills and goals', () => {
    expect(isTeamProfileComplete({ working_hours: null, skills: [], goals: 'wachsen' })).toBe(
      false,
    );
    expect(isTeamProfileComplete({ working_hours: null, skills: ['löten'], goals: null })).toBe(
      false,
    );
    expect(isTeamProfileComplete({ working_hours: null, skills: ['löten'], goals: '  ' })).toBe(
      false,
    );
    expect(
      isTeamProfileComplete({ working_hours: null, skills: ['löten'], goals: 'wachsen' }),
    ).toBe(true);
  });
});
