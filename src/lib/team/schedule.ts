import { TIMECARD_ENTRY_CATEGORIES, type TimecardEntryCategory } from '@/config/timecards';
...
export const EMPTY_WEEKLY_SCHEDULE: WeeklySchedule = {
  version: 1,
  days: Object.fromEntries(WEEKDAY_IDS.map((day) => [day, { ...DEFAULT_DAY }])) as Record<
    WeekdayId,
    WeeklyScheduleDay
  >,
};
...
function cloneSchedule(schedule: WeeklySchedule): WeeklySchedule {
  return {
    version: 1,
    days: Object.fromEntries(WEEKDAY_IDS.map((day) => [day, { ...schedule.days[day] }])) as Record<
      WeekdayId,
      WeeklyScheduleDay
    >,
  };
...
function normalizeDay(input: unknown): WeeklyScheduleDay {
  ...
  const category = Object.values(TIMECARD_ENTRY_CATEGORIES).includes(
...
export function buildTimecardEntriesForRange(
  ...
  while (cursor < rangeEnd) {
    const entry = buildScheduleEntryForDate(
      toISODate(cursor),
      schedule.days[weekdayIdFromDate(cursor)],
    );
