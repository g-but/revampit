...
const LOCALE_PATCHES = {
  ...
  ja: {
    'partner.intro': '第2段階で実際に関与する組織と人物 — 2023年の旧申請書類からの名前は含めない。',
    'belege.intro':
  ...
  ko: {
    'partner.intro': '2단계에서 실제 기여하는 조직과 사람 — 2023년 구 신청 서류의 이름은 제외.',
    'belege.intro':
...
function isArchivePartnerGroup(g) {
  ...
  if (
    /extern.*expert|expertennetzwerk|réseau.*experts|rete.*esperti|red.*expertos|外部専門|외부.*전문|внешн.*эксперт/i.test(
      g.title ?? '',
    )
  ) {
    return true;
  ...
  if (
    g.intro &&
    /Klimastiftung|Klimast application|Klimastiftungs-Antrag|demande.*Klimast|2023.*application/i.test(
      g.intro,
    )
  ) {
    return true;
