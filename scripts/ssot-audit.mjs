...
for (const file of files) {
  ...
  if (hardcodedContentPattern.test(relative) || hardcodedContentPattern.test(text)) {
    failures.push(
      `${relative}: remove hardcoded-content naming; content modules must not advertise hardcoding.`,
    );
  }
  ...
  if (
    ...
    warnings.push(
      `${relative}: raw color literal found; move it to the design token/color config layer.`,
    );
  }
}
